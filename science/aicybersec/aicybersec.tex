...
\title{KI-basierte Bedrohungen und moderne Cybersicherheit:\\
...
\begin{itemize}
	...
	\item \textbf{Klasse 5: Multi-zyklische Lernoptimierung.} Der Angreifer durchläuft mehrere Zyklen von Reconnaissance, Exploitation, Observation und Model Retrain, wodurch sein Modell immer genauer wird.
\end{itemize}
...
\Pr_{\text{Lernfehler}} := \Pr_{x \sim \mathcal{T}_{\mathrm{test}}} [ \hat{f}(x) \neq f^*(x) ]
\end{equation}

Äquivalent: Die \emph{Angriffserfolgswahrscheinlichkeit} ist:
\begin{equation}
p_{\KI}(m) = \Pr_{\text{Angriffserfolg}} := \Pr_{x \sim \mathcal{T}_{\mathrm{test}}} [ \hat{f}(x) = 1 \text{ und } f^*(x) = 1 ]
\end{equation}
\end{definition}
...
\begin{equation}
\Pr_{\text{Lernfehler}} \leq O\left(\sqrt{\frac{d \ln(m)}{m}} + \sqrt{\frac{\ln(1/\delta)}{m}}\right)
\end{equation}
...
\begin{equation}
\Pr[\text{Angriff erfolgreich}] \leq k \cdot \left( O\left(\sqrt{\frac{d \ln(m)}{m}}\right) + \exp(-m) \right)
\end{equation}
...
\begin{equation}
\Pr[\text{Erfolg}] \leq \sum_{i=1}^{k} \Pr[\text{Fehler auf } x_i^*] \leq k \cdot O\left(\sqrt{\frac{d \ln(m)}{m}}\right)
\end{equation}
...
\begin{equation}
\forall \, (x_1, \ldots, x_m) \in X^m : \, \Pr[f_1(x_i) = y_i] = \Pr[f_2(x_i) = y_i]
\end{equation}
\end{definition}
