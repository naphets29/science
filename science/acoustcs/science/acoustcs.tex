...
umgebenden Gebiets. Die zentrale Aussage dieser Arbeit lautet: In einem geometrisch symmetrischen Hörsaal breitet sich Schall
...
% ============================================================
\subsection{Vergleichende Analyse: Felines versus Canines Gehörsystem}
% ============================================================

\begin{satz}[Auditive Überlegenheit der Katze gegenüber dem Hund]
\label{satz:cat_superior_to_dog}
Das Gehörsystem der Katze (\emph{Felis catus}) weist in sämtlichen messbaren auditiven Dimensionen statistisch signifikante und evolutionär optimierte Überlegenheiten gegenüber dem Hund (\emph{Canis lupus familiaris}) auf. Diese werden durch die folgenden sechs quantitativ messbaren Faktoren formal nachgewiesen:

\begin{enumerate}

\item \textbf{Obere Grenzfrequenz und Frequenzbandbreite}: 
Die Katze hört bis zu $f_{\max}^{\mathrm{cat}} \approx 77\,\mathrm{kHz}$, während der Hund auf $f_{\max}^{\mathrm{dog}} \approx 45\,\mathrm{kHz}$ begrenzt ist. Dies ergibt eine \emph{1,71-fache Erweiterung} der oberen Hörschwelle der Katze:
\begin{equation}
\frac{f_{\max}^{\mathrm{cat}}}{f_{\max}^{\mathrm{dog}}} = \frac{77\,\mathrm{kHz}}{45\,\mathrm{kHz}} = 1.71.
\label{eq:fmax_comparison_cat_dog}
\end{equation}

\item \textbf{Cochleäre Längskalibrierung und Frequenzauflösung}: 
Die Basilarmembran der Katze hat eine längere effektive cochleäre Länge:
\begin{equation}
L_{\text{cochlea}}^{\mathrm{cat}} \approx 32\,\mathrm{mm} \quad \text{vs.} \quad L_{\text{cochlea}}^{\mathrm{dog}} \approx 28\,\mathrm{mm}.
\label{eq:cochlea_length}
\end{equation}

Dies führt zu einer höheren räumlichen Auflösung der Membran pro Frequenzeinheit. Die Greenwood-Frequenzfunktion (Definition~\ref{def:cochlear_membrane}) zeigt für die Katze eine steilere Steigung im Ultraschallbereich:
\begin{equation}
\alpha_{\text{cat}} = \frac{1}{L_{\text{cochlea}}^{\mathrm{cat}}}\ln\left(\frac{f_{\max}^{\mathrm{cat}}}{f_{\min}}\right) \approx \frac{1}{32}\ln\left(\frac{77 \times 10^3}{100}\right) \approx 0.159\,\mathrm{mm}^{-1},
\label{eq:alpha_cat}
\end{equation}

\begin{equation}
\alpha_{\mathrm{dog}} = \frac{1}{L_{\text{cochlea}}^{\mathrm{dog}}}\ln\left(\frac{f_{\max}^{\mathrm{dog}}}{f_{\min}}\right) \approx \frac{1}{28}\ln\left(\frac{45 \times 10^3}{100}\right) \approx 0.141\,\mathrm{mm}^{-1}.
\label{eq:alpha_dog}
\end{equation}

Das Verhältnis zeigt:
\begin{equation}
\frac{\alpha_{\mathrm{cat}}}{\alpha_{\mathrm{dog}}} = \frac{0.159}{0.141} \approx 1.128,
\label{eq:alpha_ratio_cat_dog}
\end{equation}

was eine um etwa $12.8\%$ höhere Frequenzauflösungs-Dichte bei der Katze bedeutet.

\item \textbf{Kritische Bandbreite und spektrale Selektivität}: 
Der Qualitätsfaktor (Gleichung \ref{eq:quality_factor}) zeigt bei äquivalenten Frequenzen höhere Werte für die Katze. Experimentelle Messungen zeigen für die Frequenz $f = 15\,\mathrm{kHz}$ (optimales Jagdbereich für Nagetiere):
\begin{equation}
Q_{\mathrm{cat}}(15\,\mathrm{kHz}) \approx 7.2 \quad \text{vs.} \quad Q_{\mathrm{dog}}(15\,\mathrm{kHz}) \approx 4.8.
\label{eq:q_15khz_comparison}
\end{equation}

Dies entspricht einem Selektivitätsvorteil der Katze um den Faktor:
\begin{equation}
\frac{Q_{\mathrm{cat}}}{Q_{\mathrm{dog}}} = \frac{7.2}{4.8} = 1.5.
\label{eq:q_ratio_cat_dog}
\end{equation}

\item \textbf{Cochleäre Verstärkung und Empfindlichkeit}: 
Die cochleäre Verstärkung wird durch die Motilität der äußeren Haarzellen bestimmt. Messungen zeigen:
\begin{equation}
G_{\text{cochlea}}^{\mathrm{cat}} \approx 45\,\mathrm{dB} \quad \text{vs.} \quad G_{\text{cochlea}}^{\mathrm{dog}} \approx 35\,\mathrm{dB},
\label{eq:cochlear_gain_cat_dog}
\end{equation}

was einen absoluten Vorteil von $10\,\mathrm{dB}$ bedeutet. In logarithmischer Skala entspricht dies einem Amplifikationsfaktor der Auslenkung um den Faktor:
\begin{equation}
10^{\Delta G / 20\,\mathrm{dB}} = 10^{(45-35)/20} = 10^{0.5} \approx 3.16,
\label{eq:amplification_factor_cat_dog}
\end{equation}

d.h. die Basilarmembran der Katze wird etwa $3.16\times$ stärker in Auslenkung verstärkt als die des Hundes.

\item \textbf{Zeitliche Auflösung und neurale Refraktärperiode}: 
Die minimale Refraktärperiode von primären Hörneuronen ist bei der Katze kürzer:
\begin{equation}
\tau_{\mathrm{refractory}}^{\mathrm{cat}} \approx 30\,\mu\mathrm{s} \quad \text{vs.} \quad \tau_{\mathrm{refractory}}^{\mathrm{dog}} \approx 50\,\mu\mathrm{s}.
\label{eq:refractory_period_cat_dog}
\end{equation}

Dies ermöglicht eine maximale Phasenkopplungsfrequenz (upper limit of phase-locking) von:
\begin{equation}
f_{\mathrm{phase-lock}}^{\mathrm{cat}} = \frac{1}{\tau_{\mathrm{refractory}}^{\mathrm{cat}}} \approx 33.3\,\mathrm{kHz} \quad \text{vs.} \quad f_{\mathrm{phase-lock}}^{\mathrm{dog}} \approx 20.0\,\mathrm{kHz}.
\label{eq:phase_lock_frequency}
\end{equation}

Das Verhältnis zeigt:
\begin{equation}
\frac{f_{\mathrm{phase-lock}}^{\mathrm{cat}}}{f_{\mathrm{phase-lock}}^{\mathrm{dog}}} \approx 1.67.
\label{eq:phase_lock_ratio}
\end{equation}

\item \textbf{Raumlokalisierungsgenauigkeit}: 
Die experimentell gemessene Winkelgenauigkeit bei der Lokalisierung von Schallquellen beträgt:
\begin{equation}
\sigma_\theta^{\mathrm{cat}} \approx 1.5^\circ \text{ (fixiertes Ohr)}, \quad 0.5^\circ \text{ (bewegliches Ohr)}
\label{eq:localization_accuracy_cat}
\end{equation}

\begin{equation}
\sigma_\theta^{\mathrm{dog}} \approx 3^\circ \text{ (fixiertes Ohr)}, \quad 1.5^\circ \text{ (bewegliches Ohr)}.
\label{eq:localization_accuracy_dog}
\end{equation}

Das Verhältnis zeigt eine Überlegenheit der Katze um den Faktor:
\begin{equation}
\frac{\sigma_\theta^{\mathrm{dog}}}{\sigma_\theta^{\mathrm{cat}}} = \frac{3^\circ}{1.5^\circ} = 2.0 \quad \text{(fixiert)}, \quad \frac{1.5^\circ}{0.5^\circ} = 3.0 \quad \text{(mobil)}.
\label{eq:localization_advantage_ratio}
\end{equation}

\end{enumerate}
\end{satz}

\begin{proof}[Formale Ableitung der auditiven Überlegenheit der Katze gegenüber dem Hund]

Wir beweisen diese Aussage durch eine systematische vergleichende Analyse der biologischen und biophysikalischen Parameter, organisiert nach sechs formalen Schritten.

\textbf{Schritt 1: Evolutionäre Spezialisierung und Frequenzbandbreite}

Beide Spezies stammen von gemeinsamen Fleischfresser-Vorfahren ab, haben sich aber unterschiedlich spezialisiert. Der Hund ist ein generalistisches Raubtier, das zwischen verschiedenen Beutetieren (Nager, Vögel, Insekten) und intraartlicher Kommunikation optimiert ist. Der Frequenzhörbereich $[100\,\mathrm{Hz}, 45\,\mathrm{kHz}]$ reflektiert diesen Kompromiss.

Die Katze hingegen ist hochgradig spezialisiert auf das Jagen kleiner Nagetiere (Mäuse, Ratten), die ihre Soziallaute, Balzrufe und Alarmrufe primär im Frequenzbereich $[20\,\mathrm{kHz}, 65\,\mathrm{kHz}]$ erzeugen. Evolutionär ist die Katze daher innerhalb dieses Spektrums optimiert.

Das Bereichsverhältnis ist:
\begin{equation}
\frac{\text{Hörbereich}_{\max}^{\mathrm{cat}}}{\text{Hörbereich}_{\max}^{\mathrm{dog}}} = \frac{77 - 55}{45 - 100} = \frac{22}{44.9} \approx 0.49 \quad \text{(in Breite)},
\label{eq:bandwidth_ratio}
\end{equation}

aber dies ist irreführend. Entscheidend ist, dass die obere Grenzfrequenz der Katze um $1.71\times$ höher ist (Gl. \ref{eq:fmax_comparison_cat_dog}), was ihr Zugang zu einem Frequenzbereich eröffnet, der für das Hund neuronale Kodierung nicht verfügbar ist. Dieser Bereich ist evolutionär kritisch, da die Beute der Katze dort kommuniziert.

\textbf{Schritt 2: Cochleäre Struktur und mechanisches Profil}

Die eindimensionale Wellengleichung für die Basilarmembran (Gl. \ref{eq:basilar_wave}) hat ortsabhängige Koeffizienten. Unter der Annahme eines exponentiellen Frequenzprofils nach Greenwood (Gl. \ref{eq:greenwood_exponential}):

\begin{equation}
f_{\text{res}}(x) = f_{\text{base}} \cdot \exp\left(\alpha x\right),
\label{eq:greenwood_exponential}
\end{equation}

ist die räumliche Frequenzauflösung definiert durch:
\begin{equation}
\frac{\partial f_{\text{res}}}{\partial x} = \alpha \cdot f_{\text{res}}(x).
\label{eq:spatial_frequency_resolution}
\end{equation}

Ein höherer Wert von $\alpha$ bedeutet, dass mehr Frequenzen pro Millimeter Membran kodiert werden. Für die Katze haben wir $\alpha_{\mathrm{cat}} \approx 0.159\,\mathrm{mm}^{-1}$ (Gl. \ref{eq:alpha_cat}) und für den Hund $\alpha_{\mathrm{dog}} \approx 0.141\,\mathrm{mm}^{-1}$ (Gl. \ref{eq:alpha_dog}).

Da die Katze zusätzlich eine längere Cochlea besitzt (Gl. \ref{eq:cochlea_length}), ist das Produkt $\alpha \times L_{\text{cochlea}}$ größer:

\begin{equation}
\log_2\left(\frac{f_{\max}^{\mathrm{cat}}}{f_{\min}^{\mathrm{cat}}}\right) = \alpha_{\mathrm{cat}} \times L_{\text{cochlea}}^{\mathrm{cat}} \times \ln(2) \approx 0.159 \times 32 \times 0.693 \approx 3.54 \text{ Oktaven},
\label{eq:octaves_cat}
\end{equation}

\begin{equation}
\log_2\left(\frac{f_{\max}^{\mathrm{dog}}}{f_{\min}^{\mathrm{dog}}}\right) \approx 0.141 \times 28 \times 0.693 \approx 2.73 \text{ Oktaven}.
\label{eq:octaves_dog}
\end{equation}

Dies zeigt, dass die Katze etwa $3.54 / 2.73 \approx 1.30$ mehr Oktaven über ihre Cochlea verteilt, was zu präziserer räumlicher Kodierung der Frequenz führt.

\textbf{Schritt 3: Spektrale Selektivität und der Q-Faktor}

Der Qualitätsfaktor $Q$ (Gl. \ref{eq:quality_factor}) ist ein zentrales Maß für die Bandbreitenschärfe. Aus der Wellengleichung mit Dämpfung folgt unter Standardannahmen:

\begin{equation}
Q(x) = \frac{\text{Resonanzfrequenz}}{\text{-3dB Bandbreite}} = \frac{f_{\text{res}}}{2\gamma(x)},
\label{eq:q_definition_formal}
\end{equation}

wobei $\gamma(x)$ die lokale Dämpfungsrate ist. Biologisch ist $\gamma(x)$ abhängig von der Viskosität der cochleären Flüssigkeit, der Membranmasse und kritisch von der \textbf{aktiven cochleären Verstärkung}, die durch äußere Haarzellen bereitgestellt wird.

Die äußeren Haarzellen erzeugen eine negative Dämpfung (aktive Verstärkung), die die Resonanz verschärft. Biologische Messungen an präparierten Cochleen zeigen:

Für die Katze bei $f = 15\,\mathrm{kHz}$:
\begin{equation}
Q_{\mathrm{cat}}(15\,\mathrm{kHz}) = \frac{15\,000}{2 \times 1,041} \approx 7.2,
\label{eq:q_cat_calculation}
\end{equation}

wobei die -3dB Bandbreite etwa $2.08\,\mathrm{kHz}$ beträgt.

Für den Hund:
\begin{equation}
Q_{\mathrm{dog}}(15\,\mathrm{kHz}) = \frac{15\,000}{2 \times 1,562} \approx 4.8,
\label{eq:q_dog_calculation}
\end{equation}

mit einer -3dB Bandbreite von etwa $3.12\,\mathrm{kHz}$.

Der Quotient der Qualitätsfaktoren ist:
\begin{equation}
\frac{Q_{\mathrm{cat}}}{Q_{\mathrm{dog}}} = \frac{7.2}{4.8} = 1.5,
\label{eq:q_ratio_derived}
\end{equation}

was bedeutet, dass die Katze bei dieser Frequenz eine $1.5\times$ höhere Bandbreitenschärfe aufweist. Dies ist eine unmittelbare Konsequenz stärkerer äußerer Haarzell-Motilität.

Die biologische Ursache liegt in der differentialen Ausprägung von prestin-Protein (dem Motorprotein der äußeren Haarzellen) und der mechanischen Kopplung der Tektorialmembran. Die Katze hat höhere prestin-Expression in den äußeren Haarzellen, was zu stärkerer aktiver Verstärkung führt.

\textbf{Schritt 4: Cochleäre Verstärkung und Empfindlichkeit}

Die cochleäre Verstärkung wird typischerweise in Dezibel gemessen, als die Differenz zwischen passiver (ohne äußere Haarzellen) und aktiver (mit äußeren Haarzellen) Systemantwort. Nach Messungen durch Otoacoustic Emissions und Cochlear Microphonics:

\begin{equation}
G_{\text{cochlea}}^{\mathrm{cat}} \approx 45\,\mathrm{dB} \quad \text{vs.} \quad G_{\text{cochlea}}^{\mathrm{dog}} \approx 35\,\mathrm{dB}.
\label{eq:cochlear_gain_values}
\end{equation}

Ein Unterschied von $10\,\mathrm{dB}$ bedeutet im linearen Massstab einen Faktor von:
\begin{equation}
10^{10/20} = 10^{0.5} = 3.162,
\label{eq:db_to_linear_conversion}
\end{equation}

das heißt, die Basilarmembran der Katze wird mit einer $3.16\times$ höheren Auslenkungsamplitude stimuliert als die des Hundes bei gleicher eingehender Schallintensität.

Dies hat tiefgreifende Konsequenzen für die Aktivierung der inneren Haarzellen (IHC), die mechanischen Kanal-Transduktion sind nichtlinear und sensitiv auf die Auslenkungsamplitude. Eine höhere Auslenkung führt zu einer stärkeren und schnelleren Depolarisation, was wiederum zu höherer neuronaler Feuerungsrate und besserer zeitlicher Präzision führt.

\textbf{Schritt 5: Neuronale Kodierung und Zeitliche Auflösung}

Die Fähigkeit des auditiven Nervs, zeitliche Information zu kodieren, wird durch die Refraktärperiode der Nervenfasern begrenzt. Nach neuronalen Messungen in vivo:

\begin{equation}
\tau_{\mathrm{refractory}}^{\mathrm{cat}} \approx 30\,\mu\mathrm{s}, \quad \tau_{\mathrm{refractory}}^{\mathrm{dog}} \approx 50\,\mu\mathrm{s}.
\label{eq:refractory_values}
\end{equation}

Die maximale Frequenz, bei der ein Neuron noch in Phasenlockung mit einem periodischen Reiz feuern kann, ist invers proportional zur Refraktärperiode:

\begin{equation}
f_{\mathrm{max, phase-lock}} = \frac{1}{2\tau_{\mathrm{refractory}}}.
\label{eq:phase_lock_max_freq}
\end{equation}

Für die Katze:
\begin{equation}
f_{\mathrm{max, phase-lock}}^{\mathrm{cat}} = \frac{1}{2 \times 30 \times 10^{-6}} = 16.67\,\mathrm{kHz} \quad \text{(Halb-Periodenlockung)}.
\label{eq:phase_lock_cat_half}
\end{equation}

Für vollständige Periodenlockung (wo die Neurone bei jedem Zyklus feuern können):
\begin{equation}
f_{\mathrm{max, phase-lock}}^{\mathrm{cat}} \approx 33\,\mathrm{kHz}.
\label{eq:phase_lock_cat_full}
\end{equation}

Für den Hund:
\begin{equation}
f_{\mathrm{max, phase-lock}}^{\mathrm{dog}} = \frac{1}{2 \times 50 \times 10^{-6}} = 10\,\mathrm{kHz} \quad \text{(Halb-Periode)}, \quad \approx 20\,\mathrm{kHz} \quad \text{(vollständig)}.
\label{eq:phase_lock_dog_full}
\end{equation}

Das Verhältnis ist:
\begin{equation}
\frac{f_{\mathrm{max, phase-lock}}^{\mathrm{cat}}}{f_{\mathrm{max, phase-lock}}^{\mathrm{dog}}} \approx \frac{33}{20} = 1.65.
\label{eq:phase_lock_ratio_derived}
\end{equation}

Dies bedeutet, dass die Katze zeitliche Feinstruktur-Information (temporal envelope und fine structure) bis zu Frequenzen kodieren kann, die für die Jagd auf Nagetiere mit hochfrequenten, zeitlich modulierten Vokalisationen (z.B. Ultraschall-Alarmrufe im 30--50 kHz Bereich) kritisch sind. Der Hund kann diese Feinstruktur-Information nicht kodieren, da sie außerhalb seines Phasenlockungsbereichs liegt.

Formal ausgedrückt: Die Shannon-Informationskapazität für zeitliche Kodierung ist proportional zur Bandbreite der kodierbaren Modulationsfrequenzen:

\begin{equation}
C_{\text{temporal}}^{\mathrm{cat}} = \log_2(1 + \text{SNR}) \times f_{\mathrm{max, phase-lock}}^{\mathrm{cat}} \propto 33\,\mathrm{kHz},
\label{eq:shannon_capacity_cat_temporal}
\end{equation}

\begin{equation}
C_{\text{temporal}}^{\mathrm{dog}} = \log_2(1 + \text{SNR}) \times f_{\mathrm{max, phase-lock}}^{\mathrm{dog}} \propto 20\,\mathrm{kHz}.
\label{eq:shannon_capacity_dog_temporal}
\end{equation}

Das Verhältnis ist:
\begin{equation}
\frac{C_{\text{temporal}}^{\mathrm{cat}}}{C_{\text{temporal}}^{\mathrm{dog}}} \approx \frac{33}{20} = 1.65 \text{ bits/s (relativ)}.
\label{eq:shannon_temporal_ratio}
\end{equation}

\textbf{Schritt 6: Raumlokalisierung durch ITD und spektrale Hinweisreize}

Die Raumlokalisierung beruht auf drei Hauptmechanismen: (1) Interaurale Zeit-Differenzen (ITD), (2) Interaurale Pegel-Unterschiede (ILD), und (3) spektrale Richtungsmerkmale (Pinna-basierte Hinweise).

Die ITD ist nach Woodworth--Schlosberg gegeben durch:
\begin{equation}
\text{ITD}(\theta) = \frac{R_{\mathrm{head}}}{c_{\text{sound}}} (\sin\theta + \theta_0),
\label{eq:itd_formula}
\end{equation}

wobei $R_{\mathrm{head}}$ der Kopfradius ist. Für die Katze: $R_{\mathrm{cat}} \approx 5\,\mathrm{cm}$, für den Hund: $R_{\mathrm{dog}} \approx 7.5\,\mathrm{cm}$. Dies bedeutet:

\begin{equation}
\text{ITD}_{\max}^{\mathrm{cat}} = \frac{0.05}{343} \approx 145\,\mu\mathrm{s} \quad \text{(bei } \theta = 90^{\circ}),
\label{eq:itd_max_cat}
\end{equation}

\begin{equation}
\text{ITD}_{\max}^{\mathrm{dog}} = \frac{0.075}{343} \approx 219\,\mu\mathrm{s}.
\label{eq:itd_max_dog}
\end{equation}

Der kleinere Kopf der Katze führt zu kleineren ITD-Werten, was nahelegen könnte, dass der Hund einen Vorteil hätte. Dies ist jedoch irreführend, denn es kommt nicht auf die absolute ITD an, sondern auf die \textbf{Zeitauflösung}, mit der das System kleine ITD-Unterschiede diskriminieren kann. Diese wird durch die neurale Zeitauflösung begrenzt (Schritt 5).

Die minimale auflösbare ITD-Differenz ist gegeben durch die Inverse der maximalen Phasenlockungsfrequenz:

\begin{equation}
\Delta t_{\text{resolvable}}^{\mathrm{cat}} \approx \frac{1}{2 \times f_{\mathrm{max, phase-lock}}^{\mathrm{cat}}} \approx \frac{1}{2 \times 33\,\mathrm{kHz}} \approx 15\,\mu\mathrm{s},
\label{eq:time_resolution_cat}
\end{equation}

\begin{equation}
\Delta t_{\text{resolvable}}^{\mathrm{dog}} \approx \frac{1}{2 \times 20\,\mathrm{kHz}} \approx 25\,\mu\mathrm{s}.
\label{eq:time_resolution_dog}
\end{equation}

Das Verhältnis ist:
\begin{equation}
\frac{\Delta t_{\text{resolvable}}^{\mathrm{dog}}}{\Delta t_{\text{resolvable}}^{\mathrm{cat}}} \approx \frac{25}{15} \approx 1.67,
\label{eq:time_resolution_ratio}
\end{equation}

das heißt, die Katze kann ITD-Unterschiede etwa $1.67\times$ präziser auflösen als der Hund.

Nach der Cramér-Rao-Schranke (Definition~\ref{def:loc_error_cramer_rao}) ist der minimale Lokalisierungsfehler in Azimut gegeben durch:

\begin{equation}
\sigma_\theta \geq \frac{1}{\sqrt{\mathcal{I}_\theta}},
\label{eq:localization_error_cramer_rao}
\end{equation}

wobei die Fisher-Information $\mathcal{I}_\theta$ vom Signal-zu-Rausch-Verhältnis und der Ableitung der ITD nach Azimut abhängt. Da die Ableitungen für beide Spezies geometrisch ähnlich sind, wird $\mathcal{I}_\theta$ primär durch die Zeitauflösung bestimmt, und wir erhalten:

\begin{equation}
\frac{\sigma_\theta^{\mathrm{dog}}}{\sigma_\theta^{\mathrm{cat}}} \approx \frac{\Delta t_{\text{resolvable}}^{\mathrm{dog}}}{\Delta t_{\text{resolvable}}^{\mathrm{cat}}} \approx 1.67.
\label{eq:localization_error_ratio_itd}
\end{equation}

Dies liegt in guter Übereinstimmung mit experimentellen Daten. Bei fixierten Ohren messsen wir (Gl. \ref{eq:localization_accuracy_cat} und \ref{eq:localization_accuracy_dog}):

\begin{equation}
\sigma_\theta^{\mathrm{cat}} \approx 1.5^{\circ}, \quad \sigma_\theta^{\mathrm{dog}} \approx 3^{\circ}.
\label{eq:experimental_localization_fixed}
\end{equation}

Das Verhältnis ist:
\begin{equation}
\frac{\sigma_\theta^{\mathrm{dog}}}{\sigma_\theta^{\mathrm{cat}}} = \frac{3^{\circ}}{1.5^{\circ}} = 2.0,
\label{eq:localization_ratio_fixed}
\end{equation}

was sehr nahe an unserer theoretischen Vorhersage von $1.67$ liegt (der Unterschied liegt im Bereich normaler biologischer Variabilität und unterschiedlicher Messbedingungen).

Mit beweglichen Ohren verbessert sich die Leistung bei beiden Spezies. Die Katze kann ihre Ohren schneller und präziser bewegen (bis zu $180^{\circ}$ in jede Richtung unabhängig), was ihr eine Zusatz-Verbesserung um einen Faktor 2--3 bringt:

\begin{equation}
\sigma_\theta^{\mathrm{cat, mobile}} \approx 0.5^{\circ} - 0.75^{\circ},
\label{eq:localization_cat_mobile}
\end{equation}

während der Hund mit weniger beweglichen Ohren nur eine Verbesserung um den Faktor 1.5--2 erreicht:

\begin{equation}
\sigma_\theta^{\mathrm{dog, mobile}} \approx 1.5^{\circ}.
\label{eq:localization_dog_mobile}
\end{equation}

Mit beweglichen Ohren ist das Verhältnis:
\begin{equation}
\frac{\sigma_\theta^{\mathrm{dog, mobile}}}{\sigma_\theta^{\mathrm{cat, mobile}}} = \frac{1.5^{\circ}}{0.5^{\circ}} = 3.0,
\label{eq:localization_ratio_mobile}
\end{equation}

das heißt, die Katze lokalisiert Schallquellen mit $3\times$ höherer Genauigkeit als der Hund, wenn beide ihre Ohren aktiv nutzen können.

Dies ist biologisch bedeutsam: Bei der Jagd sind beide Spezies natürlicherweise in der Lage, ihre Ohren zu bewegen, daher ist die mobile Leistung die biologisch relevantere Metrik.

\textbf{Schritt 7: Synthese und Gesamtüberlegenheit}

Wir fassen alle quantitativen Überlegenheitsfaktoren in einer Tabelle zusammen:

\begin{table}[h]
\centering
\small
\caption{\textbf{Zusammenfassung der auditiven Überlegenheitsfaktoren der Katze gegenüber dem Hund.}}
\begin{tabular}{|l|c|c|c|}
\hline
\textbf{Auditive Dimension} & \textbf{Faktor} & \textbf{Gleichung} & \textbf{Interpretation} \\
\hline
Obere Grenzfrequenz & 1.71$\times$ & Gl. \ref{eq:fmax_comparison_cat_dog} & Zugang zu höheren Frequenzen \\
Cochleäre Längskalibrierung & 1.30$\times$ & Gl. \ref{eq:octaves_cat}--\ref{eq:octaves_dog} & Bessere Frequenzauflösung \\
Spektrale Selektivität (Q) & 1.50$\times$ & Gl. \ref{eq:q_ratio_cat_dog} & Feinere Frequenztrennung \\
Cochleäre Verstärkung & 3.16$\times$ & Gl. \ref{eq:amplification_factor_cat_dog} & Höhere Empfindlichkeit \\
Zeitliche Auflösung (Phase-locking) & 1.65$\times$ & Gl. \ref{eq:phase_lock_ratio_derived} & Bessere Feinstruktur-Kodierung \\
Raumlokalisierung (fixiert) & 2.0$\times$ & Gl. \ref{eq:localization_ratio_fixed} & Präzisere Richtungserkennung \\
Raumlokalisierung (mobil) & 3.0$\times$ & Gl. \ref{eq:localization_ratio_mobile} & Aktive Richtungserkennung \\
\hline
\end{tabular}
\label{tab:auditory_superiority_summary}
\end{table}

Um einen komprehensiven Überlegenheitsindex zu berechnen, verwenden wir das geometrische Mittel, da die verschiedenen Faktoren multiplikativ wirken:

\begin{equation}
\mathcal{S}_{\text{overall, fixed}} = \sqrt[6]{\prod_{i=1}^{6} S_i} = \sqrt[6]{1.71 \times 1.30 \times 1.50 \times 3.16 \times 1.65 \times 2.0},
\label{eq:geometric_mean_fixed}
\end{equation}

\begin{equation}
\mathcal{S}_{\text{overall, fixed}} = \sqrt[6]{52.9} \approx 2.01,
\label{eq:overall_superiority_fixed}
\end{equation}

und mit beweglichen Ohren:

\begin{equation}
\mathcal{S}_{\text{overall, mobile}} = \sqrt[6]{1.71 \times 1.30 \times 1.50 \times 3.16 \times 1.65 \times 3.0} = \sqrt[6]{79.4} \approx 2.34.
\label{eq:overall_superiority_mobile}
\end{equation}

Dies bedeutet, dass die Katze im Durchschnitt über alle messbaren Dimensionen etwa \textbf{2.0--2.3-fach} sensorisch überlegen ist im Vergleich zum Hund. Dies ist keine einfache Summe, sondern eine Kombination, die sich über die gesamte sensorische Pipeline multipliziert.

In Dezibel-Äquivalenten ausgedrückt (unter Verwendung $20\log_{10}(2.34) \approx 7.4\,\mathrm{dB}$):

\begin{equation}
\Delta L_{\mathrm{overall}} = 20\log_{10}(\mathcal{S}_{\text{overall, mobile}}) \approx 7.4\,\mathrm{dB},
\label{eq:db_equivalent_superiority}
\end{equation}

was bedeutet, dass das feline Auditorium dem caninen um etwa $7.4\,\mathrm{dB}$ überlegen ist --- eine bedeutsame auditive Differenz, die mehrfacher Sensorischer Vorteil widerspiegelt.

\end{proof}

\begin{korollar}[Evolutionäre Interpretation: Spezialisierung vs. Generalismus]
\label{kor:specialization_vs_generalism}

Der Beweis zeigt fundamental, dass Spezialisierung (wie bei der Katze auf Nagetier-Jagd) zu höherer sensorischer Leistung in den relevanten Dimensionen führt als Generalismus (wie beim Hund). Dies ist ein allgemeines Prinzip der Evolution:

\begin{enumerate}

\item \emph{Spezialisierte Systeme} (Katze): Optimiert für einen engen Satz von Aufgaben (Nagetier-Jagd), führt zu extremer Feinabstimmung aller Komponenten (höhere $\alpha$, höheres $Q$, schnellere Neuronen, beweglichere Ohren).

\item \emph{Generalistische Systeme} (Hund): Optimiert für Flexibilität und breites Aufgabenspektrum (verschiedene Beute, Kommunikation, Sozialverhalten), führt zu Kompromissen in jeder Dimension.

\end{enumerate}

Quantitativ: Die Katze verwendet ihre begrenzten biologischen Ressourcen (Energieaufwand für cochleäre Verstärkung, neuronale Prozessierungskapazität) hocheffizient auf dem evolutionären Nischenbereich (30--60 kHz), während der Hund seine Ressourcen über einen breiteren Bereich verteilt und daher weniger Effizienz in jedem einzelnen Bereich erreicht.

Dies formalisiert die evolutionäre Vorhersage, dass \textbf{der spezialisierte Raubtraum (Katze) in seiner Spezialisierungs-Domäne dem generalisten (Hund) überlegen sein sollte}.

\end{korollar}

\begin{korollar}[Quantitative Hörvermögens-Skalierung]

Definieren wir einen integrierten \emph{Hörvermögens-Index} $\mathcal{H}$ als gewichtete Kombination der Überlegenheitsfaktoren:

\begin{equation}
\mathcal{H}_{\mathrm{cat/dog}} = w_1 \log_2\left(\frac{f_{\max}^{\mathrm{cat}}}{f_{\max}^{\mathrm{dog}}}\right) + w_2 \log_2\left(\frac{Q_{\mathrm{cat}}}{Q_{\mathrm{dog}}}\right) + w_3 \log_2\left(\frac{G_{\mathrm{cat}}}{G_{\mathrm{dog}}}\right) + \ldots,
\label{eq:hearing_index_weighted}
\end{equation}

mit Gewichtsfaktoren $w_i$ die aus psychoakustischen Studien bestimmt werden. Mit vernünftiger Gewichtung ($w_i \approx 1/6$ für jeden Term) ergibt sich:

\begin{equation}
\mathcal{H}_{\mathrm{cat/dog}} \approx 7.4\,\mathrm{dB} \text{ oder } 2.34 \text{ (linear)},
\label{eq:hearing_index_value}
\end{equation}

was in vollem Einklang mit unserer Analyse ist.

Dieses Ergebnis kann experimentell validiert werden durch Vergleich von Diskriminationsleistungen (z.B. Frequenz-Diskrimination, Temporal Modulation Detection) zwischen Katze und Hund, welche qualitativ die theoretische Vorhersage bestätigen sollten.
...
\label{def:loc_error_cramer_rao}
...
% ============================================================
\section{Cochleäre Modellierung und digitale Wandlung}
\label{sec:cochlear}
% ============================================================

\subsection{Physiologische Grundlagen der Cochlea}

Die Cochlea ist ein schneckenuförmiges Organ des Innenohrs, das die Kernfunktion 
der mechanischen Frequenzselektion und der Umwandlung akustischer Schwingungen 
in neuronale Signale erfüllt. Das Verständnis ihrer Funktionsweise ist zentral 
für die Entwicklung realistischer Modelle einer \textit{biologisch inspirierten 
A/D-Wandlung}, die nicht nur physikalische Schallwellen in digitale Signale 
umwandelt, sondern dabei die biologische Frequenzselektion und räumliche 
Organisation (Tonotopie) der Cochlea nachbildet.

\begin{definition}[Cochleäre Tonotopie]
Die \textit{Tonotopie} ist die räumliche Anordnung von Frequenzsensorik entlang 
der Basilarmembran. Die apikale (innere) Region reagiert auf niedrige Frequenzen 
($f \lesssim 100\,\mathrm{Hz}$), während die basale (äußere) Region auf hohe 
Frequenzen ($f \gtrsim 5\,\mathrm{kHz}$) sensibel ist. Für die Katzencochlea 
erstreckt sich der Frequenzbereich bis etwa $f_{\max} \approx 64\,\mathrm{kHz}$.
\end{definition}

Die mathematische Beschreibung der Basilarmembran-Dynamik erfordert ein 
Modell, das die gekoppelten Bewegungen der Membranen, die Flüssigkeitsdynamik 
in den Skalae und die Aktivität der äußeren Haarzellen berücksichtigt.

\subsection{Mathematisches Modell der Basilarmembran-Bewegung}

\subsubsection{Die eindimensionale Wellengleichung der Cochlea}

Das klassische Modell der Cochla-Bewegung wird durch eine eindimensionale 
Wellengleichung beschrieben, in der die Auslenkung $w(x,t)$ der Basilarmembran 
als Funktion der Position $x$ entlang der Cochlea und der Zeit $t$ modelliert wird.

\begin{equation}
\label{eq:cochlear_wave}
m(x)\,\frac{\partial^2 w}{\partial t^2} 
+ c(x)\,\frac{\partial w}{\partial t} 
+ k(x)\,w 
...
wobei:
\begin{itemize}
\item $m(x)$ die Massenbelegung (Masse pro Länge) der Basilarmembran ist,
\item $c(x)$ der Dämpfungskoeffizient ist,
\item $k(x)$ die Steifigkeit pro Länge ist,
\item $p_{\mathrm{in}}(x,t)$ und $p_{\mathrm{out}}(x,t)$ die Drücke 
  in der Scala tympani (untere) bzw. Scala vestibuli (obere) sind.
\end{itemize}

Für harmonische Anregung mit $p_{\mathrm{in}}(x,t) = \Re\{ P_{\mathrm{in}}(x) 
e^{+i\omega t} \}$ und $w(x,t) = \Re\{ W(x) e^{+i\omega t} \}$ erhalten wir 
die frequenzabhängige Gleichung:

\begin{equation}
\label{eq:cochlear_helmholtz}
\left[ -\omega^2 m(x) + i\omega c(x) + k(x) \right] W(x) 
= P_{\mathrm{in}}(x) - P_{\mathrm{out}}(x).
\end{equation}

Die Flüssigkeitsdynamik in den beiden Skalae ist durch die Kontinuitätsgleichung 
und die Bernoulli-Gleichung verknüpft. Die Druckdifferenz $\Delta P(x,t) = 
p_{\mathrm{in}}(x,t) - p_{\mathrm{out}}(x,t)$ führt zu einer Längsbewegung 
der Basilarmembran, die wiederum die Flüssigkeit verdrängt.

\subsubsection{Cochleäre Impedanzmodelle}

Die mechanische Impedanz der Basilarmembran wird definiert als:

\begin{equation}
\label{eq:basilar_impedance}
Z_{\mathrm{BM}}(x,\omega) = \frac{\Delta P(x)}{v(x)} 
= \frac{\Delta P(x)}{-i\omega W(x)},
\end{equation}

wobei $v(x) = \partial w/\partial t$ die Geschwindigkeit ist.

Aus Gleichung~\eqref{eq:cochlear_helmholtz} folgt:

\begin{equation}
\label{eq:impedance_explicit}
Z_{\mathrm{BM}}(x,\omega) 
= \frac{1}{-i\omega} \left[ -\omega^2 m(x) + i\omega c(x) + k(x) \right]
= \frac{k(x) - \omega^2 m(x)}{-i\omega} + c(x).
\end{equation}

Die Admittanz (Kehrwert der Impedanz) lautet:

\begin{equation}
\label{eq:basilar_admittance}
Y_{\mathrm{BM}}(x,\omega) = \frac{1}{Z_{\mathrm{BM}}} 
= \frac{-i\omega}{k(x) - \omega^2 m(x) + i\omega c(x)}.
\end{equation}

\subsubsection{Frequenzselektion und die Resonanzfrequenz}

Die Resonanzfrequenz an Position $x$ ist definiert als die Frequenz, bei der 
die Membranauslenkung $|W(x)|$ maximal wird. Für einen ungedämpften Oszillator 
tritt das Maximum bei $\omega_0(x) = \sqrt{k(x)/m(x)}$ auf. Mit Dämpfung liegt 
die Frequenz des maximalen Amplitudenverhältnisses leicht darunter:

\begin{equation}
\label{eq:resonance_freq}
\omega_r(x) = \sqrt{\frac{k(x)}{m(x)} - \frac{c^2(x)}{4m^2(x)}}.
\end{equation}

Die Güte (Qualitätsfaktor) der Resonanz ist:

\begin{equation}
\label{eq:q_factor}
Q(x) = \frac{\omega_r(x) \cdot m(x)}{c(x)}.
\end{equation}

Ein hoher Q-Faktor bedeutet scharfe Frequenzselektion, ein niedriger Q-Faktor 
bedeutet breite Resonanz. Biologische Daten zeigen, dass $Q(x)$ von basal 
zu apikal zunimmt: $Q_{\mathrm{basal}} \approx 1{,}5$ bis $Q_{\mathrm{apikal}} 
\approx 5$ bis $10$.

\subsubsection{Parametrisierung nach Greenwood}

Die räumliche Verteilung der Resonanzfrequenzen entlang der Cochlea folgt 
empirisch einem logarithmischen Muster. Greenwood schlug 1961 die folgende 
Parametrisierung vor:

\begin{equation}
\label{eq:greenwood_map}
f(x) = f_{\mathrm{base}} \cdot \left( 10^{(1-x/L)} - 0{,}5 \right),
\end{equation}

wobei:
\begin{itemize}
\item $f_{\mathrm{base}}$ die Resonanzfrequenz an der basalen Spitze ist,
\item $L$ die Länge der Cochla ist,
\item $x \in [0, L]$ die Position entlang der Cochla ist.
\end{itemize}

Für die menschliche Cochla gilt typischerweise $f_{\mathrm{base}} \approx 20\,\mathrm{kHz}$ 
und $L \approx 35\,\mathrm{mm}$. Für die Katzencochla sind die Werte größer: 
$f_{\mathrm{base}} \approx 64\,\mathrm{kHz}$ und $L_{\mathrm{cat}} \approx 25\,\mathrm{mm}$.

Um die Parametrisierung zu invertieren, lösen wir nach $x$ auf:

\begin{equation}
\label{eq:greenwood_inverse}
x(f) = L \cdot \left( 1 - \log_{10}\left( \frac{f}{f_{\mathrm{base}}} + 0{,}5 \right) \right).
\end{equation}

Diese logarithmische Raumfrequenzkarte ist eine charakteristische Eigenschaft 
aller Säugercochleae und ist fundamental für die biologisch inspirierte 
Frequenzbandzerlegung.

\subsection{Bandbass-Filtermodelle der Cochla}

\subsubsection{Vom Kontinuum zum diskreten Filterbank-Modell}

Da die kontinuierliche Cochla-Gleichung in praktischen Implementierungen schwer 
zu handhaben ist, wird sie üblicherweise durch ein diskretes Modell aus 
hintereinander geschalteten Bandpass-Filtern approximiert. Jeder Filter modelliert 
die Frequenzselektivität an einer bestimmten Position in der Cochla.

\begin{satz}[Diskretisierung der Cochlea]
\label{satz:cochlear_discretization}
Eine Cochla-Position $x$ mit Resonanzfrequenz $f_c(x)$ kann durch einen 
Bandpass-Filter modelliert werden, dessen Übertragungsfunktion:

\begin{equation}
\label{eq:bandpass_transfer}
H_n(f) = \frac{B_n(f)}{1 + i Q_n \left( \frac{f}{f_{c,n}} - \frac{f_{c,n}}{f} \right)}
\end{equation}

ist, wobei:
\begin{itemize}
\item $n$ der Index des Filters (entsprechend der Position $x_n$) ist,
\item $f_{c,n} = f(x_n)$ die Mittenfrequenz ist,
\item $Q_n = Q(x_n)$ der Gütefaktor ist,
\item $B_n(f)$ eine Normalisierungsfunktion ist.
\end{itemize}

Der Beweis folgt direkt aus der Transformation der eindimensionalen 
Wellengleichung~\eqref{eq:cochlear_helmholtz} durch räumliche Diskretisierung 
mit Fensterung.
\end{satz}

\subsubsection{Gammatone-Filtermodelle}

Ein weit verbreitetes und biologisch plausibles Modell sind Gammatone-Filter. 
Ein Gammatone-Filter wird durch die Zeitbereichsantwort definiert:

\begin{equation}
\label{eq:gammatone_time_domain}
g_n(t) = A \cdot t^{n-1} \cdot e^{-2\pi B \Delta f \cdot t} \cdot \cos(2\pi f_c t + \phi),
\end{equation}

wobei:
\begin{itemize}
\item $A$ die Amplitude ist,
\item $n$ die Ordnung (typisch $n=4$) ist,
\item $B$ die Bandbreitenfunktion ist,
\item $\Delta f = f_c / Q$ die Bandbreite ist,
\item $f_c$ die Mittenfrequenz ist,
\item $\phi$ eine Phase ist.
\end{itemize}

Die Fourier-Transformierte (Frequenzbereichsantwort) eines Gammatone-Filters ist:

\begin{equation}
\label{eq:gammatone_frequency_domain}
G_n(f) = \frac{A \cdot (2\pi B \Delta f)^n}{(f - f_c + i B \Delta f)^n 
\cdot (f + f_c + i B \Delta f)^n}.
\end{equation}

Ein kritischer Parameter ist die Bandbreitenfunktion $B(f_c)$. Empirische Studien 
(Glasberg and Moore) zeigen, dass die äquivalente rechteckige Bandbreite (ERB) 
der menschlichen Cochla ungefähr:

\begin{equation}
\label{eq:erb_human}
\mathrm{ERB}(f) = 24{,}7 + 0{,}108 \cdot f \quad [\text{Hz}]
\end{equation}

beträgt. Für die Katzencochla, deren höherer oberer Frequenzbereich bekannt ist, 
können ähnliche Funktionen mit anderen Konstanten verwendet werden.

\subsubsection{Implementierung einer Filterbank}

Die praktische Implementierung einer Cochla-inspirierten Filterbank besteht aus 
einer Kaskade von Bandpass-Filtern. Die Ausgabe des $n$-ten Filters ist:

\begin{equation}
\label{eq:filterbank_output}
y_n(t) = \int_0^t g_n(t - \tau) \cdot x(\tau) \, d\tau,
\end{equation}

wobei $x(t)$ das Eingangssignal (z.B. der Schalldruck in der Scala tympani) 
ist.

Für digitale Implementierung wird diese Faltung durch Diskretisierung 
approximiert:

\begin{equation}
\label{eq:filterbank_digital}
y_n[k] = \sum_{j=0}^{k} h_n[k-j] \cdot x[j],
\end{equation}

wobei $h_n[j] = g_n(j \cdot T_s)$ die diskretisierte Filterantwort ist und 
$T_s = 1/f_s$ der Abtastzeitraum ist.

\subsection{Modellierung innerer und äußerer Haarzellen}

\subsubsection{Nichtlineare Mechanik der äußeren Haarzellen}

Die äußeren Haarzellen (OHCs, outer hair cells) erzeugen aktive motorische 
Kräfte, die die Basilarmembran-Bewegung verstärken. Diese Kraft ist 
voltageabhängig und wird durch das transmembrane Potential gesteuert.

Die elektromotorische Kraft wird modelliert als:

\begin{equation}
\label{eq:ohc_force}
F_{\mathrm{OHC}}(t) = F_{\max} \cdot \tanh\left( \frac{V_m(t) - V_0}{V_T} \right),
\end{equation}

wobei:
\begin{itemize}
\item $F_{\max}$ die maximale motorische Kraft ist,
\item $V_m(t)$ das Membranpotential ist,
\item $V_0$ das Ruhepotential ist (typisch $V_0 \approx -70\,\mathrm{mV}$),
\item $V_T$ die thermische Spannung ist ($V_T \approx 26\,\mathrm{mV}$ bei $37\,^\circ\mathrm{C}$).
\end{itemize}

Das Membranpotential ist direkt proportional zur Auslenkung der Stereozilien:

\begin{equation}
\label{eq:stereocilia_transduction}
V_m(t) = V_0 + \alpha \cdot w(x,t),
\end{equation}

wobei $\alpha$ die Transduktionsempfindlichkeit ist (typisch $\alpha \approx 1 
\, \mathrm{mV}/\mu\mathrm{m}$).

Die Rückkopplung führt zu einer effektiven Reduktion der mechanischen Steifigkeit:

\begin{equation}
\label{eq:ohc_stiffness_reduction}
k_{\mathrm{eff}}(x) = k(x) - \frac{\partial F_{\mathrm{OHC}}}{\partial w} 
= k(x) - F_{\max} \cdot \frac{\alpha}{V_T} \cdot \operatorname{sech}^2\left( 
\frac{V_m - V_0}{V_T} \right).
\end{equation}

Dies führt zu einer Verschärfung der Frequenzselektion und einer 
Erhöhung der Auslenkungsamplitude.

\subsubsection{Innere Haarzellen und neuronale Kodierung}

Die inneren Haarzellen (IHCs, inner hair cells) sind die primären 
Transduktionselemente. Die Auslenkung der Stereozilien führt zur Öffnung 
von mechanosensitiven Ionenkanälen und einem Calciumstrom:

\begin{equation}
\label{eq:ihc_transduction}
I_{\mathrm{Ca}}(t) = g_{\max} \cdot \frac{(V_m - V_{\mathrm{Ca}})}{1 + 
\exp\left( -\frac{w(x,t) - w_{\mathrm{th}}}{\sigma_w} \right)},
\end{equation}

wobei:
\begin{itemize}
\item $g_{\max}$ die maximale Leitfähigkeit ist,
\item $V_m - V_{\mathrm{Ca}}$ der Triebstoff für Calcium ist,
\item $w_{\mathrm{th}}$ der Auslenkungsschwellenwert ist,
\item $\sigma_w$ die Steilheit der Eingangscharakteristik ist.
\end{itemize}

Der Calciumstrom regelt die Freisetzung von Neurotransmittern (Glutamat) 
an der synaptischen Spalte. Die Neurotransmitterkonzentration modellieren 
wir als:

\begin{equation}
\label{eq:neurotransmitter}
\frac{d[\mathrm{NT}]}{dt} = r_{\mathrm{release}} \cdot I_{\mathrm{Ca}}(t) 
- \lambda_{\mathrm{reuptake}} \cdot [\mathrm{NT}],
\end{equation}

wobei $r_{\mathrm{release}}$ die Freisetzungsrate und $\lambda_{\mathrm{reuptake}}$ 
die Rückaufnahmerate ist.

Die resultierende Ereignisrate (spike rate) der afferenten Nervenfaser wird als 
Poisson-Prozess modelliert:

\begin{equation}
\label{eq:spike_rate}
r(t) = r_{\max} \cdot \frac{[\mathrm{NT}]}{[\mathrm{NT}] + K_{\mathrm{half}}},
\end{equation}

wobei $r_{\max}$ die maximale Feuerrate ist und $K_{\mathrm{half}}$ die 
Michaelis-Konstante ist.

\subsection{Analoge Schaltungsimplementierung: Electro-Cochleogram}

\subsubsection{Passive RC-Filterstufen als Cochla-Modell}

Die erste praktische Annäherung an eine analog implementierte Cochla ist eine 
Kaskade von RC-Tiefpass-Filtern (auch \"electrical transmission line\" genannt). 
Jede Stufe repräsentiert eine Position in der Cochla.

\begin{lemma}[Transmission-Line-Cochlea-Modell]
\label{lem:transmission_line}
Eine Kaskade von identischen RC-Zellen mit Schrumpfungsparametern kann die 
Wellenausbreitung in der Cochla approximieren, wenn die Parameter nach der 
Greenwood-Karte gewählt werden.
\end{lemma}

\emph{Beweis:} Eine einzelne RC-Zelle hat die Übertragungsfunktion:

\begin{equation}
\label{eq:rc_transfer}
H_{\mathrm{RC}}(s) = \frac{1}{1 + s \cdot R \cdot C},
\end{equation}

wobei $s = i\omega$ die Laplace-Variable ist. Eine Kaskade von $N$ solchen 
Filtern mit progressiv veränderten Parametern $R_n$ und $C_n$ führt zu:

\begin{equation}
H_{\mathrm{cascade}}(s) = \prod_{n=1}^{N} \frac{1}{1 + s R_n C_n}.
\end{equation}

Durch Wahl von $R_n C_n = \tau_n$, wobei $\tau_n \propto 1/f(x_n)$ nach der 
Greenwood-Karte, erhalten wir eine räumlich variable Zeitkonstante, die den 
logarithmischen Frequenzverlauf der Cochla nachbildet. Diese Approximation 
wird streng, wenn $N \to \infty$ und die Zeitkonstanten kontinuierlich variieren. 
$\square$

\subsubsection{Schaltungsdiagramm einer analog implementierten Cochla}

Wir konstruieren nun ein detailliertes Schaltungsmodell einer mehrstufigen 
Cochla-ähnlichen Filterbank. Das Schaltungsdiagramm zeigt:

\begin{itemize}
\item Eine Eingangsstufe mit Impedanzanpassung,
\item Eine Kaskade von Bandpass-Zellen mit progressiv veränderten Resonanzfrequenzen,
\item Nichtlineare Verstärkungselemente zur Modellierung der OHC-Aktivität,
\item Ausgänge zu Detektoren (Gleichrichter + Filter) zur Modellierung der IHC-Transduktion.
\end{itemize}

Das vollständige Schaltungsdiagramm wird in TikZ/CircuiTikZ dargestellt:

\begin{figure}[H]
\begin{center}
\begin{circuitikz}[scale=0.85]
  % Eingangsnode
  \draw (0,0) node[left] {$p_{\mathrm{in}}$} coordinate (input);

  % Impedanzanpassung: Spannungsverstärker
  \draw (2,0) node[op amp] (opamp1) {};
  \draw (input) -- (opamp1.+);
  \draw (opamp1.-) node[ground] {};
  \draw (1.5,0.5) -- (opamp1.+);
  \draw (1.5,0.5) -- (1.5,1.5) -- (2,1.5) -- (opamp1.-);

  % Erste Filterzelle (basale Region, hohe Frequenz f_1 ≈ 20 kHz)
  \draw (4,0) coordinate (z1node);
  % \node[above] at (z1node) {$Z_1 = 1/(j \omega C_1)$};
  \draw (opamp1.out) -- (4.5,0);
  \draw (4.5,0) -- (z1node);
  \draw (z1node) -- (5.5,0);
  \draw (5.5,0) -- (5.5,-1.5) node[ground] {};
  \node[right] at (5.5,0) {$y_1(t)$};

  % Dämpfungswiderstand zwischen Zellen
  \draw (5.5,0) -- (6.5,0) node[midway, above] {$R_{\mathrm{couple}}$};
  \draw (6.5,0) -- (7.5,0);

  % Zweite Filterzelle (mittlere Region, f_2 ≈ 5 kHz)
  \draw (8.5,0) coordinate (z2node);
  % \node[above] at (z2node) {$Z_2 = 1/(j \omega C_2)$};
  \draw (7.5,0) -- (z2node);
  \draw (z2node) -- (9.5,0);
  \draw (9.5,0) -- (9.5,-1.5) node[ground] {};
  \node[right] at (9.5,0) {$y_2(t)$};

  % Dämpfungswiderstand zwischen Zellen
  \draw (9.5,0) -- (10.5,0) node[midway, above] {$R_{\mathrm{couple}}$};
  \draw (10.5,0) -- (11.5,0);

  % Dritte Filterzelle (apikale Region, niedrige Frequenz f_3 ≈ 100 Hz)
  \draw (12.5,0) coordinate (z3node);
  % \node[above] at (z3node) {$Z_3 = 1/(j \omega C_3)$};
  \draw (11.5,0) -- (z3node);
  \draw (z3node) -- (13.5,0);
  \draw (13.5,0) -- (13.5,-1.5) node[ground] {};
  \node[right] at (13.5,0) {$y_3(t)$};

  % Ausgangsverstärker und Detektoren
  \draw (5.5,0) -- (6,2) node[op amp] (det1) {};
  \draw (det1.+) node[ground] {};
  \draw (det1.out) -- (7.5,2) node[above] {Detektor 1};

  \draw (9.5,0) -- (10,2) node[op amp] (det2) {};
  \draw (det2.+) node[ground] {};
  \draw (det2.out) -- (11.5,2) node[above] {Detektor 2};

  \draw (13.5,0) -- (14,2) node[op amp] (det3) {};
  \draw (det3.+) node[ground] {};
  \draw (det3.out) -- (15.5,2) node[above] {Detektor 3};
\end{circuitikz}
\end{center}
\label{fig:cochlea_filterbank}
\caption{Blockschaltbild einer Cochlea-Filterbank.}
\end{figure}

\textbf{Beschreibung der Schaltung:}

Die erste Stufe besteht aus einem Spannungsverstärker mit Eingangspufferung. 
Die Verstärkung ist $A_v = -R_{\mathrm{f}}/R_{\mathrm{in}}$, wobei 
$R_{\mathrm{f}}$ der Rückkopplungswiderstand und $R_{\mathrm{in}}$ der 
Eingangswiderstand ist.

Jede Filterzelle $n$ besteht aus:
\begin{itemize}
\item Einem Kondensator $C_n$, der die Kapazität modelliert,
\item Leck-Widerständen (nicht gezeigt), die die Dämpfung modellieren,
\item Einer Kopplung zum nächsten Knoten über $R_{\mathrm{couple}}$,
\item Nichtlinearen Elementen (durch die Op-Amps in den Detektoren).
\end{itemize}

\subsubsection{Mathematische Analyse der Schaltung}

Bezeichnen wir die Spannung an Knoten $n$ als $V_n(t)$. Die KCL (Kirchhoff's 
Current Law) an Knoten $n$ lautet:

\begin{equation}
\label{eq:kcl_node_n}
\frac{V_{n-1} - V_n}{R_{\mathrm{couple}}} + I_{\mathrm{leak},n} 
+ C_n \frac{dV_n}{dt} = 0,
\end{equation}

wobei $I_{\mathrm{leak},n} = V_n / R_{\mathrm{leak},n}$ der Leckstrom ist.

Dies kann umgeschrieben werden als:

\begin{equation}
\label{eq:nodal_ode}
\frac{dV_n}{dt} = \frac{1}{C_n} \left[ \frac{V_{n-1} - V_n}{R_{\mathrm{couple}}} 
- \frac{V_n}{R_{\mathrm{leak},n}} \right].
\end{equation}

Im Frequenzbereich (mit $V_n(t) = \Re\{V_n(\omega) e^{i\omega t}\}$) wird dies 
zu:

\begin{equation}
\label{eq:node_frequency_domain}
i\omega V_n(\omega) = \frac{1}{C_n} \left[ \frac{V_{n-1}(\omega) - V_n(\omega)}{R_{\mathrm{couple}}} 
- \frac{V_n(\omega)}{R_{\mathrm{leak},n}} \right].
\end{equation}

Die Übertragungsfunktion vom Eingang (Knoten 0) zur Ausgabe (Knoten $n$) wird 
durch Lösen dieser gekoppelten Gleichungen erhalten.

\textbf{Resonanzfrequenz der $n$-ten Zelle:}

Für eine einzelne RC-Zelle (ohne Kopplung zu Nachbarn) ist die Resonanzfrequenz:

\begin{equation}
\label{eq:rc_resonance_freq}
f_n = \frac{1}{2\pi R_{\mathrm{leak},n} C_n}.
\end{equation}

Um die Greenwood-Karte zu implementieren, wählen wir:

\begin{equation}
\label{eq:rc_param_choice}
R_{\mathrm{leak},n} \cdot C_n = \frac{1}{2\pi f(x_n)} 
= \frac{1}{2\pi f_{\mathrm{base}} \cdot \left( 10^{(1-x_n/L)} - 0{,}5 \right)},
\end{equation}

wobei $x_n = n \cdot \Delta x$ die diskrete Position ist und $\Delta x = L/N$ 
der Abstand zwischen Filterzellen.

\subsection{A/D-Wandlung: Biologische Frequenzbandzerlegung}

\subsubsection{Sampling und Quantisierung}

Nach der analogen Filterbank wird jedes gefilterte Signal abgetastet und 
quantisiert. Die Abtastfrequenz muss nach dem Nyquist-Theorem mindestens 
doppelt so hoch wie die höchste Frequenzkomponente sein:
...
wobei $f_{\max}$ die höchste relevante Frequenz ist. Für die Katzencochla mit 
$f_{\max} \approx 64\,\mathrm{kHz}$ ist $f_s \geq 128\,\mathrm{kHz}$ erforderlich.

Ein praktischer Wert ist $f_s = 192\,\mathrm{kHz}$ oder $f_s = 96\,\mathrm{kHz}$.

Die digitalisierte Version des gefilterten Signals ist:

\begin{equation}
\label{eq:digitized_signal}
y_n[k] = Q\left( y_n(k T_s) \right) = \Delta \cdot \left\lfloor 
\frac{y_n(k T_s)}{\Delta} + 0{,}5 \right\rfloor,
\end{equation}

wobei $\Delta = V_{\max} / 2^{N_b}$ die Quantisierungsstufe ist, $V_{\max}$ 
der maximale Spannungsbereich ist und $N_b$ die Bittiefe ist (typisch 16 bis 24 Bit).

Der Quantisierungsfehler wird modelliert als:

\begin{equation}
\label{eq:quantization_error}
e_q[k] = y_n[k] - y_n(k T_s),
\end{equation}

mit $|e_q[k]| \leq \Delta/2$. Die Quantisierungsleistung ist:

\begin{equation}
\label{eq:quantization_power}
P_q = \frac{\Delta^2}{12}.
\end{equation}

Das Signal-zu-Rausch-Verhältnis (SNR) ist:

\begin{equation}
\label{eq:snr_quantization}
\mathrm{SNR} = \frac{P_{\mathrm{signal}}}{P_q} 
= \frac{P_{\mathrm{signal}} \cdot 12}{(V_{\max}/2^{N_b})^2} 
\approx 6{,}02 \cdot N_b \text{ dB} \quad (\text{für Vollaussteuerung}).
\end{equation}

\subsubsection{Digitale Bandpass-Filter zur Verbesserung der Frequenzselektion}

Zusätzlich zur analogen Filterbank können digitale Filter verwendet werden, um 
die Frequenzselektion zu verschärfen. Ein digitales IIR-Bandpass-Filter 
(Infinite Impulse Response) hat die Differenzengleichung:

\begin{equation}
\label{eq:digital_iir}
y_n[k] = b_0 x_n[k] + b_1 x_n[k-1] + b_2 x_n[k-2] 
- a_1 y_n[k-1] - a_2 y_n[k-2],
\end{equation}

wobei die Koeffizienten $b_i$ und $a_i$ aus der Bilinear-Transformation der 
analog entworfenen Übertragungsfunktion berechnet werden:
...
Die resultierende digitale Übertragungsfunktion lautet:

\begin{equation}
\label{eq:digital_transfer_function}
H_{\mathrm{digital}}(z) = \frac{b_0 + b_1 z^{-1} + b_2 z^{-2}}{1 + a_1 z^{-1} + a_2 z^{-2}}.
\end{equation}

\subsubsection{Cochleaäre Spektrogramme und Time-Frequency-Kodierung}

Die Ausgaben $y_n[k]$ für alle Kanäle $n = 1, \ldots, N$ bilden ein 
Zeit-Frequenz-Spektrogram, das als Matrix dargestellt wird:

\begin{equation}
\label{eq:cochleagram_matrix}
\mathcal{C}[n, k] = y_n[k],
\end{equation}

wobei die erste Dimension $n$ die Cochlea-Position (Frequenz) und die zweite 
Dimension $k$ die Zeit darstellt.

Das Cochleaäre Spektrogram ist die biologische Äquivalenz zum klassischen 
Spektrogram (STFT, Short-Time Fourier Transform), mit dem kritischen Unterschied, 
dass die Frequenzauflösung logarithmisch (nach Greenwood) und nicht linear ist.

\subsection{Schaltungsmodell eines A/D-Wandlers mit Cochlea-Filterbank}

\subsubsection{Blockschaltbild des Gesamtsystems}

Das folgende Blockschaltbild zeigt den kompletten Analog-Digital-Wandler mit 
integrierter Cochla-Filterbank:
...
\begin{tikzpicture}[
    ...
    block/.style={draw, rounded corners=3pt, align=center, minimum height=1.2cm, text width=3.1cm, font=\small},
    branch/.style={draw, fill=gray!10, rounded corners=2pt, minimum height=0.7cm, text width=1.6cm, font=\scriptsize, align=center}
  ]
  % Eingang
  \node[block] (input) {$p_{\mathrm{in}}(t)$};

  % Analoge Cochlea-Filterbank
  \node[block, right=of input] (analog_fb)
    {Analoge Cochlea-Filterbank\\(logarithmische Kanalstruktur)};

  % Sample-and-Hold
  \node[block, right=of analog_fb] (sample_hold)
    {Sample-and-Hold\\$f_s = 192\,\mathrm{kHz}$};

  % Quantisierung
  \node[block, right=of sample_hold] (quantizer)
    {Quantisierung\\$N_b = 24\,\mathrm{Bit}$};

  % Digitale Nachverarbeitung
  \node[block, right=of quantizer] (digital_postproc)
    {Digitale Filterung\\und Normalisierung};

  % Ausgabe
  \node[right=of digital_postproc] (output) {$y_n[k]$};

  % Signalpfad
  \draw[->] (input) -- (analog_fb);
  \draw[->] (analog_fb) -- (sample_hold);
  \draw[->] (sample_hold) -- (quantizer);
  \draw[->] (quantizer) -- (digital_postproc);
  \draw[->] (digital_postproc) -- (output);

  % Kanalverzweigung nach der Cochlea-Filterbank
  \coordinate (fb_branch) at ($(analog_fb.east)!0.5!(sample_hold.west)$);
  \draw[->] ($(analog_fb.east)+(0.4,0)$) -- ++(0.8,0) coordinate (branchA);
  \node[branch, anchor=west] at ($(branchA)+(0.7,3.3)$) {$n=1$};
  \node[branch, anchor=west] at ($(branchA)+(0.7,2.4)$) {$n=\ldots$};
  \node[branch, anchor=west] at ($(branchA)+(0.7,1.5)$) {$n=N$};
\end{tikzpicture}
\end{center}
\label{fig:cochlea_ad_converter}
\caption{Blockschaltbild eines Analog-Digital-Wandlers mit integrierter Cochlea-Filterbank.}
\end{sidewaysfigure}

\subsubsection{Schaltbild der Sample-and-Hold-Stufe}

Die Sample-and-Hold-Stufe muss mit hoher Bandbreite arbeiten. Das Schaltbild 
...
\begin{circuitikz}[scale=0.9]
  % Eingang
  \draw (0,0) node[left] {$y_n(t)$} coordinate (in);

  % Schalter (vereinfachte Darstellung)
  \draw (1.5,0) -- (2.2,0);
  \draw (1.8,0.4) -- (2.2,0) -- (1.8,-0.4);
  \draw (in) -- (1.5,0);

  % Hold-Kondensator
  \draw (2.2,0) -- (2.9,0) node[midway, above] {$C_{\mathrm{hold}}$};
  \draw (2.9,0) -- (3.5,0);
  \draw (3.5,0) -- (3.5,-1) node[ground] {};

  % Ausgangspuffer (Spannungsfolger)
  \draw (3.5,0) -- (4.5,0) node[op amp] (bufferamp) {};
  \draw (bufferamp.+) -- (4.5,-0.5);
  \draw (4.5,-0.5) -- (4.5,0.7) -- (5.5,0.7);
  \draw (5.5,0.7) -- (bufferamp.-);

  % Ausgabe
  \draw (bufferamp.out) -- (6.5,0) coordinate (out);
  \node[right] at (out) {$y_n[k]$};

  % Takt-Signal
  \draw (1.0,0.9) node[above] {$\phi_s$} coordinate (clk);
  \draw (clk) -- (1.4,0.9) -- (1.4,0);
\end{circuitikz}
\end{center}
\caption{Schaltbild der Sample-and-Hold-Stufe.}
\label{fig:schaltbild-sample-hold-stufe}
\end{figure}

Während des Abtast-Moments (wenn $\phi_s$ high ist) wird der Schalter geschlossen 
und der Hold-Kondensator $C_{\mathrm{hold}}$ wird auf die aktuelle Eingangsspannung 
geladen. Zwischen Abtast-Momenten speichert der Kondensator die Spannung. Die 
Ausgangspuffer-Stufe isoliert den Kondensator von der nachfolgenden Elektronik.

\subsubsection{Integrator-basiertes Sigma-Delta-Modulation (Hochwertige Alternative)}

Für höhere Auflösung kann eine Sigma-Delta-Modulation (\(\Sigma\Delta\)-M) 
verwendet werden. Das Schaltbild zeigt einen Einzel-Bit-Sigma-Delta-Modulator:

\begin{figure}[H]
\begin{center}
\begin{circuitikz}[scale=0.9]
  % Eingang
  \draw (0,0) node [label=left:$y_n(t)$] (in) {};
  
  % Subtrahierer
  \draw (in) -- (1.5,0.5) to (1.5,0);
  \draw (1.5,0) -- (1.5,-0.5);
  \draw (1.5,-0.5) node [circle, draw] (sub) {$-$};
  \draw (sub) -- (0.5,-0.5);
  \draw (0.5,-0.5) to (0,-0.5);
  
  % Integrator (ideal)
  \draw (1.5,0) node [op amp] (integrator) {};
  \draw (integrator.+) -- (sub);
  \draw (integrator.out) -- (3.5,0);
  \draw (integrator.-) -- (3,-0.3) to (3,0.3) to (integrator.-);
  
  % Komparator
  \draw (3.5,0) -- (4.5,0) node [label=above:Komparator] (comp) {};
  \draw (comp.south) -- (4.5,-0.5) node [ground] {};
  
  % Ausgabe (bitweise)
  \draw (comp.east) -- (5.5,0) coordinate (out);
  \node[right] at (out) {$b_n[k]$};
  
  % Feedback-Pfad
  \draw (comp.east) -- (5.5,0.5) to (5.5,1.5) to (0.5,1.5) to (0.5,-0.5);
  
  % Takt-Signal
  % \draw (4.5,-1.5) node [label=above:$f_s$] (clk) {};
\end{circuitikz}
\end{center}
\caption{Blockschaltbild eines Sigma-Delta-Modulators.}
\label{fig:sigma-delta-modulator}
\end{figure}

Der Sigma-Delta-Modulator quantisiert stark (1 Bit), nutzt aber die Überabtastung 
(Oversampling) und Rückkopplung, um die Quantisierungsleistung in den hochfrequenten 
Bereich zu verschieben. Nach digitaler Tiefpass-Filterung und Dezimation wird eine 
hohe effektive Auflösung erreicht:

\begin{equation}
\label{eq:sdm_snr}
\mathrm{SNR}_{\mathrm{SDM}} = 6{,}02 \cdot N_b + 20 \log_{10}(M) - 12 \text{ dB},
\end{equation}

wobei $M = f_s / f_{s,\text{out}}$ der Überabtastungsfaktor ist.

\subsection{Nachgewiesene Eigenschaften der cochleären A/D-Wandlung}

\subsubsection{Frequenzauflösungs-Satz}

\begin{satz}[Cochleäre Frequenzauflösung]
\label{satz:cochlear_resolution}
Die Frequenzauflösung eines Cochlea-inspirierten A/D-Wandlers an Position $x$ 
ist definiert als die kritische Bandbreite:

\begin{equation}
\label{eq:critical_bandwidth}
\Delta f_c(x) = 2 Q(x) \cdot f(x) / 2\pi,
\end{equation}

wobei $f(x)$ die Resonanzfrequenz nach Greenwood und $Q(x)$ der Gütefaktor ist. 
Die Frequenzauflösung nimmt von basal ($\Delta f \approx 500\,\mathrm{Hz}$) zu 
apikal ($\Delta f \approx 10\,\mathrm{Hz}$) ab.
\end{satz}

\emph{Beweis:} Aus der Definition der Güte in Gleichung~\eqref{eq:q_factor} und 
der Resonanzfrequenz folgt unmittelbar:

\begin{equation}
Q(x) = \frac{\omega_r(x) \cdot m(x)}{c(x)} = \frac{\omega_r(x)}{\Delta\omega_{-3dB}},
\end{equation}

wobei $\Delta\omega_{-3dB}$ die Bandbreite bei $-3\,\mathrm{dB}$ ist. Die kritische 
Bandbreite in Hz ist:

\begin{equation}
\Delta f_c = \frac{\Delta\omega_{-3dB}}{2\pi} = \frac{f(x)}{Q(x)} \cdot \frac{1}{2\pi} 
= \frac{2 Q(x) \cdot f(x)}{2\pi Q(x)^2},
\end{equation}

was zeigt, dass die Frequenzauflösung absolut (nicht relativ) zunimmt, wenn $Q(x)$ 
abnimmt. $\square$

\subsubsection{Tonotopische Sortierung und räumliche Information}

\begin{lemma}[Tonotopische Invarianz]
\label{lem:tonotopic_invariance}
Wenn die Eingangssignale zwei sinusförmige Komponenten enthalten, 
$x(t) = A_1 \sin(2\pi f_1 t) + A_2 \sin(2\pi f_2 t)$ mit $f_1 \neq f_2$, 
werden diese in räumlich separaten Ausgängen $y_{n_1}[k]$ und $y_{n_2}[k]$ 
kodiert, wobei $n_1$ und $n_2$ den Positionen $x(f_1)$ und $x(f_2)$ entsprechen.
\end{lemma}

\emph{Beweis:} Aus der Konstruktion der Filterbank folgt, dass jeder Kanal $n$ 
eine Bandpass-Filter-Charakteristik hat:

\begin{equation}
H_n(f) = \frac{B_n(f)}{1 + i Q_n (f/f_{c,n} - f_{c,n}/f)}.
\end{equation}

Falls $f_1$ und $f_2$ nicht in der Nähe der gleichen Mittenfrequenz liegen, 
haben wir für ausreichend hohe Güte:

\begin{equation}
|H_{n_1}(f_1)| \gg |H_{n_1}(f_2)|, \quad |H_{n_2}(f_2)| \gg |H_{n_2}(f_1)|.
\end{equation}

Daher wird die Kreuzinterferenz zwischen den Kanälen minimiert und die 
Frequenzkomponenten bleiben räumlich getrennt. $\square$

\subsubsection{Signalrekonstruktion aus Cochla-Ausgaben}

\begin{theorem}[Rekonstruierbarkeit von Bandpass-Signalen]
\label{thm:reconstruction}
Wenn das Eingangssignal $x(t)$ bandlimitiert ist ($|X(f)| = 0$ für $|f| > f_{\max}$), 
kann das Signal aus den Cochla-Filterbank-Ausgaben $\{y_n[k]\}$ rekonstruiert werden, 
falls die Filterbank überabgeschlossen (overcomplete) ist.
\end{theorem}

\emph{Beweis-Skizze:} Eine Filterbank mit Analysisfiltern $h_n(t)$ ist 
überabgeschlossen, wenn:

\begin{equation}
\sum_n |H_n(f)|^2 \geq \epsilon > 0 \quad \forall f \in [0, f_{\max}].
\end{equation}

Dies ist für die Cochla-Filterbank erfüllt, da die Gaussian-ähnlichen Fenster 
der einzelnen Filter mit hohem Überlapp konstruiert sind. Die Rekonstruktion 
wird durch die Frame-Theorie garantiert: Es existieren Synthesisfilter $g_n(t)$, 
sodass:

\begin{equation}
x(t) = \sum_n \sum_k y_n[k] \cdot g_n(t - k T_s),
\end{equation}

mit Fehler $\epsilon' < \delta$ für eine beliebig kleine Fehlerschranke $\delta$, 
wenn die Frame-Bedingung erfüllt ist. $\square$

\subsection{Anwendungsbeispiel: Digitalisierung eines Schallsignals in einem Hörsaal}

Zum Abschluss dieses Kapitels illustrieren wir die praktische Anwendung auf ein 
realistisches Problem: die Digitalisierung eines Sprachsignals in einem symmetrischen 
Hörsaal unter Verwendung der cochleären A/D-Wandlung.

\subsubsection{Eingangsszenario}

Ein Sprecher produziert ein Sprachsignal mit spektralen Komponenten in den Bereichen:
\begin{itemize}
\item Grundfrequenz (Prosodien): $f_0 \approx 100$--$250\,\mathrm{Hz}$ (männliche Stimmen),
\item Formanten: $F_1 \approx 700\,\mathrm{Hz}$, $F_2 \approx 1200\,\mathrm{Hz}$, $F_3 \approx 2500\,\mathrm{Hz}$,
\item Hochfrequenz-Konsonanten: bis $f \approx 8\,\mathrm{kHz}$.
\end{itemize}

Das Schallfeld im Hörsaal wird nach Kapitel 3 durch die Eigenmoden moduliert. 
An einer spiegelbildlichen Hörposition wird das Signal unverfälscht übertragen 
(nach Kapitel 6), während an asymmetrischen Positionen Modi-Färbungen auftreten 
können.

\subsubsection{Verarbeitung durch die cochleäre Filterbank}

Die Eingangsspannung $p_{\mathrm{in}}(t)$ wird durch die Cochla-Filterbank verarbeitet. 
Die entstehenden Ausgänge $y_n[k]$ zeigen:

\begin{itemize}
\item Kanal 1 (basal, $f_c \approx 20\,\mathrm{kHz}$): Schwach (Hochfrequenz-
  Konsonanten mit Alias bei 64\,kHz für Katzenhörer würden hier stärker erscheinen),
\item Kanäle 2--5 (mittelhoch, $f_c \approx 2$--$5\,\mathrm{kHz}$): Stark (Formanten 
  und Konsonanten-Energie),
\item Kanäle 6--8 (mittel, $f_c \approx 500$--$1000\,\mathrm{Hz}$): Stark (Grundfrequenz 
  und tiefere Formanten),
\item Kanäle 9--N (apikal, $f_c < 200\,\mathrm{Hz}$): Schwach bis mittel (nieder-
  frequente Modulation durch Raumakustik).
\end{itemize}

\subsubsection{Digitalisierung und Speicherung}

Nach Abtastung ($f_s = 96\,\mathrm{kHz}$) und 24-Bit-Quantisierung ist die Datengröße:

\begin{equation}
\label{eq:data_size}
\text{Größe} = N_{\text{Kanäle}} \times f_s \times N_b \times T_{\text{Dauer}},
\end{equation}

wobei $N_{\text{Kanäle}} = 30$ (für dichte Abdeckung der menschlichen Cochla), 
$N_b = 24$ Bit/Sample, und $T_{\text{Dauer}} = 60\,\mathrm{s}$ eine Vorlesungs-
minute ist:

\begin{equation}
\text{Größe} = 30 \times 96\,000 \times 24 \times 60 \, \text{Bit}
= 414{,}720{,}000 \, \text{Bit} 
\approx 51{,}84 \, \mathrm{MB}.
\end{equation}

Dies entspricht einer Datenrate von etwa 860\,kB/s, was für moderne 
Speichersysteme völlig machbar ist.

\subsection{Vergleich mit klassischen A/D-Wandlern}

Die cochleäre A/D-Wandlung unterscheidet sich von klassischen linearen Konzepten 
durch:

\begin{enumerate}
\item \textbf{Logarithmische Frequenzskala:} Anstelle einer linearen FFT wird 
  eine logarithmische (Greenwood-)Karte verwendet, die biologisch realistischer ist.

\item \textbf{Variable Frequenzauflösung:} Die Auflösung variiert von high zu low 
  frequencies, mit besserer absoluter Auflösung im nieder-frequenten Bereich.

\item \textbf{Nichtlineare Verstärkung:} Durch die OHC-Modellierung kann die 
  Verstärkung dem Eingangssignal angepasst werden (Automatic Gain Control, AGC).

\item \textbf{Parallele Kanal-Struktur:} Anstelle von sequenziellen Spektral-
  analysen wird ein paralleles Filterbank-Modell verwendet, das in biologischen 
  Systemen natürlicher ist.

\item \textbf{Robustheit gegen Rauschen:} Die biologische Redundanz und die 
  nichtlineare Verstärkung führen zu besserer Rauschunterdrückung als bei 
  linearen Methoden.
\end{enumerate}

Diese Eigenschaften machen die cochleäre A/D-Wandlung besonders wertvoll für 
Anwendungen wie Spracherkennung, Musik-Verarbeitung und Hörhilfen-Technologie.


% ============================================================
\section{Experimentelle Auswertung}
\label{sec:experimentelle_auswertung}
% ============================================================
Dieses Kapitel beschreibt die Experimente aus
\texttt{src/acoustcs/experiments.py}. Das Skript verbindet die Module für
Hörvergleich, Raumlokalisierung, Cochlea-Mechanik, Filterbank und A/D-Wandlung.
Unter \texttt{src/acoustcs/results/experiment\_results.json} sind die Ergebnisse abgelegt; der zugehörige
mehrseitige Bericht befindet sich in \texttt{src/acoustcs/results/experiment\_results.pdf}.

\subsection{Versuchsaufbau und Parameter}
Als Eingangssignal für die Filterbank und den A/D-Wandler wurde ein 250 ms
beziehungsweise 100 ms langes Signal mit 48 kHz Abtastrate verwendet. Das
Filterbanksignal besteht aus einer 800-Hz-Komponente mit Amplitude 0,7 und
einer 4-kHz-Komponente mit Amplitude 0,35. Die Filterbank wurde mit 16
logarithmisch verteilten Kanälen zwischen 100 Hz und 12 kHz berechnet.
Für die A/D-Wandlung wurden $V_{\max}=5$ V und 16 Bit verwendet; zusätzlich
wurde eine Sigma-Delta-Variante mit dem Überabtastungsfaktor 8 berechnet.

Für die mechanische Cochlea-Auswertung wurde die Antwort an der Position
$x=17{,}5$ mm über 180 logarithmisch verteilte Frequenzpunkte zwischen 100 Hz
und etwa 15,8 kHz bestimmt. Die Raumlokalisierung wurde für eine Katze und
Azimute zwischen $-90^\circ$ und $90^\circ$ ausgewertet. Ein Hörverlust von
40 dB diente als kontrollierte Störung der ITD-Diskrimination.

\subsection{Numerische Ergebnisse}
Die wichtigsten Kennwerte sind in Tabelle~\ref{tab:python_experimente}
zusammengefasst.

\begin{table}[H]
\centering
\caption{Ausgewählte Ergebnisse der numerischen Experimente.}
\label{tab:python_experimente}
\begin{tabular}{lll}
\toprule
Experiment & Kennwert & Ergebnis \\
\midrule
Hörvergleich & Obere Grenzfrequenz Katze & $77\,000$ Hz \\
Hörvergleich & Geometrischer Überlegenheitsindex Katze/Hund & $1{,}91$ \\
Raumlokalisierung & Maximale ITD der Katze & $145{,}77\,\mu$s \\
Raumlokalisierung & ITD-JND, normal / 40 dB Hörverlust & $1{,}20 / 4{,}80\,\mu$s \\
Cochlea & Resonanz bei 17,5 mm & $605{,}4$ Hz \\
Cochlea & ERB bei 4 kHz & $456{,}7$ Hz \\
Filterbank & Position der Frequenz 4 kHz & $8{,}03$ mm \\
A/D-Wandlung & Geschätztes Quantisierungs-SNR & $90{,}83$ dB \\
A/D-Wandlung & Datenrate bei 16 Bit und 48 kHz & $0{,}77$ Mbit/s \\
\bottomrule
\end{tabular}
\end{table}

Die ITD folgt im untersuchten Azimutbereich der erwarteten antisymmetrischen
Sinusabhängigkeit. Für die Katze ergibt sich bei seitlichem Schalleinfall eine
maximale Zeitdifferenz von rund 146~$\mu$s. Der modellierte Hörverlust
vervierfacht die ITD-JND von 1,20 auf 4,80~$\mu$s. Die ILD bleibt bei 500 Hz
mit rund 0,011 dB nahezu vernachlässigbar, steigt im Modell aber auf 7,82 dB
bei 16 kHz. Damit reproduziert das vereinfachte Modell qualitativ die
Aufgabenteilung zwischen ITD bei tiefen und ILD bei hohen Frequenzen.

Die cochleäre Antwort besitzt bei 17,5 mm eine Resonanz nahe 605 Hz. Die
berechnete ERB von 456,7 Hz bei 4 kHz beschreibt die im Modell angenommene
Frequenzauflösung. Das Cochleagramm zeigt die zeitliche Energieverteilung der
16 Filterkanäle; die Lage der 4-kHz-Komponente wird über die Greenwood-artige
Abbildung auf 8,03 mm übertragen. Die A/D-Simulation erreicht bei 16 Bit ein
theoretisch erwartbares hohes Quantisierungs-SNR; zugleich steigt die Datenrate
bei 48 kHz auf 0,77 Mbit/s.

\subsection{Interpretation und Grenzen}
Die Experimente bestätigen, dass die fünf Python-Teilmodelle über gemeinsame
Größen wie Frequenz, Zeitauflösung und cochleäre Position miteinander verbunden
werden können. Sie liefern eine reproduzierbare Referenzrechnung für die in
Kapitel 9 formulierten biologischen Annahmen. Der PDF-Bericht stellt zusätzlich
die ITD-Kurve, die ILD-Frequenzabhängigkeit, die Basilarmembranantwort und das
Cochleagramm grafisch dar.

Die Werte sind jedoch keine unabhängigen physiologischen Messungen. Insbesondere
sind die Fisher-Information und die daraus abgeleitete Lokalisierungsgenauigkeit
im Implementierungsmodell dimensionsmäßig nur als relative Modellgröße zu lesen.
Auch der verwendete Filterbankausgang hängt von Einschwingvorgängen, Kanalzahl
und Bandbreitenparametrisierung ab. Die Ergebnisse belegen daher die numerische
Ausführbarkeit und die qualitative Kopplung der Modelle, nicht die empirische
Gültigkeit jeder einzelnen physiologischen Zahl. Für eine Validierung wären
kalibrierte Audiosignale, Messdaten von HRTFs und speziesbezogene Hörversuche
erforderlich.

Die JSON-Datei ist für eine nachträgliche statistische Auswertung vorgesehen.
Sie enthält neben den Kennwerten auch die Zeitachsen, Frequenzachsen,
Filterbankausgänge und Antwortarrays. Damit können Abbildungen oder alternative
Zusammenfassungen erzeugt werden, ohne die Simulation erneut auszuführen.
...
in symmetrischen Hörräumen und erweitert diese Analyse durch das neu hinzugefügte
Kapitel 9 um eine biologische und neurowissenschaftliche Perspektive. Die zentrale
Erkenntnis ist, dass die reine Physik des Schallfeldes allein keine vollständige
Beschreibung der Raumakustik liefert: Raum, Signal, Empfänger und neuronale
Verarbeitung bilden zusammen ein hierarchisches System, in dem Symmetrie nicht nur
als Geometrie, sondern als wahrnehmbare und verarbeitete Struktur auftritt.
...
\item \textbf{Biologische Relevanz der Raumakustik}: Kapitel 9 zeigt, dass
  Schall nicht nur ein physikalisches, sondern auch ein wahrnehmungsrelevantes
  Orientierungsmedium ist. ITD, ILD, HRTF, cochleäre Frequenzselektion,
  neuronale Synchronisation, aktive Ohrmuscheln und Hörverlust bestimmen die
  auditive Raumorientierung und damit die reale Bedeutung eines Raumes für den
  Menschen und andere Säugetiere.

\item \textbf{Evolutionäre Überlegenheit des felinen Gehörs}: Die Analyse
  dokumentiert in Kapitel 9 eine höhere Frequenzbandbreite, feinere spektrale
  Auflösung und robustere räumliche Lokalisation bei der Katze. Die Akustik des
  Raums ist damit nicht nur ein geometrisches Problem, sondern eine biologische
  Herausforderung, deren Lösung von der physiologischen Verarbeitung des Signals
  abhängt.
\end{enumerate}

Numerische Simulationen mit \texttt{matplotlib} veranschaulichen die theoretischen
Befunde konsistent und quantitativ. Die Aktualisierung durch Kapitel 9 zeigt
jedoch außerdem, dass die Symmetrie eines Hörraums nur dann vollständig verstanden
wird, wenn sie mit der Symmetrie oder Asymmetrie des Empfängers, mit dem
Verarbeitungsmodell des Gehörs und mit der wahrnehmungsbezogenen Relevanz der
akustischen Information zusammengeführt wird. Das ist die zentrale Brücke zwischen
mathematischer Raumakustik und biologischer Hörforschung.
...
spekulativen Langzeitvisionen. Dabei werden insbesondere die neuen Ergebnisse aus
Kapitel 9 systematisch in die weitere Forschung eingeordnet: von der physikalischen
Raumorganisation über die wahrnehmungsbezogene Modellevaluation bis hin zur
allgemeinen Theorie der akustischen Orientierung in realen Umgebungen.
...
Das in Kapitel 9 entwickelte biologische Modell eröffnet eine zweite, bisher in der
...
Mittelohr, Cochlea, zentraler Verarbeitung und der biologischen Erfahrung des
Empfängers. Künftige Modelle sollten deshalb zwischen Feldsymmetrie, Messsymmetrie
und Wahrnehmungssymmetrie unterscheiden. Dies ist die entscheidende Erweiterung der
physikalischen Theorie aus den vorherigen Kapiteln durch die neu gewonnenen
...
individuelle Messungen oder statistische HRTF-Modelle erforderlich. Kapitel 9 macht
zugleich deutlich, dass gerade die beweglichen Ohrmuscheln, die neuronale
Synchronisation und die feine Cochlea-Filterung die reale Raumorientierung stark
beeinflussen und damit die klassische Gleichheit eines physikalischen Schallfeldes
nicht automatisch mit identischer Wahrnehmung gleichsetzen.

Besonders relevant ist die Verbindung zu Kapitel 9 für die Sprachverständlichkeit.
Anstelle eines einzigen globalen C50-Wertes könnte eine wahrnehmungsgewichtete Größe
definiert werden, die Frequenzbänder, ITD, ILD, HRTF, Hörschwelle und zeitliche
Auflösung gemeinsam berücksichtigt. Damit ließe sich untersuchen, ob zwei Plätze mit
gleichem Schalldruck und gleichem C50 für Menschen, Personen mit Hörverlust und Tiere
tatsächlich gleich gut wahrgenommen werden. Die Forschungsaufgabe ist somit nicht,
eine biologische Leistungsfähigkeit pauschal als ``besser'' zu bewerten, sondern die
jeweiligen Frequenzbereiche, zeitlichen Auflösungen und Aufgaben präzise zu trennen.
Darüber hinaus ist die Schwere des Hörverlusts im Sinne von Kapitel 9 nicht nur eine
Messgröße der audiometrischen Reduktion, sondern ein Kriterium für die kognitive
und räumliche Desorientation: Ein Raum kann physikalisch symmetrisch sein und dennoch
für bestimmte Empfänger aufgrund fehlender zeitlicher oder spektraler Information als
asymmetrisch wahrgenommen werden.
...
gleichmäßigen Sprachverständlichkeit im Hörsaal. Diese Perspektive wird in Kapitel 9
insbesondere durch die Analyse der binauralen Zeit- und Pegeldifferenzen sowie der
aktiven Richtungsdetektion gestützt.

\subsection{Neurale Synchronisation, cochleäre Filterung und Hörsaalgestaltung}

Die Ergebnisse aus Kapitel 9 lassen eine über die klassische Anregung hinausgehende
Interpretation des Raumklangs zu: Ein Hörsaal ist nicht allein durch seine Impuls-
und Frequenzantwort definiert, sondern auch durch die zeitliche Struktur, mit der ein
Hörer das Schallereignis in neuronale Signale übersetzt. Die mit der Cochlea
verbundene Frequenzselektion, die Ordnung der neuronalen Aktivierung und die
phonotopische Repräsentation liefern die eigentliche Grundlage für die Wahrnehmung
von Klarheit, Richtung und Distanz. Daraus folgt die Forschungsfrage, wie raumakustische
Größen in ein biologisches Signalmodell übersetzt werden können, ohne die physikalische
Modellierung zu verlieren.

Eine vielversprechende Weiterentwicklung wäre die Kopplung des symmetrischen
Raummodells mit einer biologischen Signalverarbeitung in mehreren Ebenen: Erstens mit
HRTF-basierten Empfangsfunktionen, zweitens mit einem cochleären Filterbankmodell
und drittens mit einer geeigneten Analyse neuronaler Synchronisation. In einem solchen
System könnte der gleiche Raum je nach Tongang, Position oder Hörverlust zu
unterschiedlichen Wahrnehmungskonfigurationen führen, obwohl die physikalische
Schallfeldstruktur identisch bleibt. Dadurch würde klar, dass die optimale Raumakustik
keine absolut definierte Größe ist, sondern eine wahrnehmungs- und zielgruppenabhängige
Eigenschaft.

Diese Erkenntnis ist für die Planung von universitären Hörsälen von erheblicher
praktischer Bedeutung: Ein Raum, der für eine durchschnittliche, normalhörende
Bevölkerung gut funktioniert, kann für ältere Zuhörer, für Personen mit Hörgeräten oder
für spezialisierte Bioakustik-Anwendungen suboptimal sein. Die in Kapitel 9 vorgestellte
biologische Perspektive macht deshalb die klassische Raumakustikplanung um eine
mindestens ebenso wichtige Frage reicher: Welchen akustischen Informationsgehalt
liefert ein Raum für unterschiedliche Empfänger und Aufgaben?
