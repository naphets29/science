...
\subsection{Digitale A/D-Wandlung mit biologischer Frequenzbandzerlegung}
...
\begin{tikzpicture}[
  ...
  \node[branch, anchor=west] at ($(branchA)+(0.7,0.9)$) {$n=1$};
  \node[branch, anchor=west] at ($(branchA)+(0.7,0.0)$) {$n=\ldots$};
  \node[branch, anchor=west] at ($(branchA)+(0.7,-0.9)$) {$n=N$};
\end{tikzpicture}
\end{center}
...
\subsection{Formal nachgewiesene Eigenschaften der cochleären A/D-Wandlung}
...
\subsection{Zusammenfassung und Vergleich mit klassischen A/D-Wandlern}
