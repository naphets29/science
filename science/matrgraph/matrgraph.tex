...
% ============================================================
\section{Formale algebraische Eigenschaften der Äquivalenz}
\label{sec:algebraische_eigenschaften}
% ============================================================

\subsection{Operationen unter Äquivalenztransformation}
\label{subsec:operationen_aequivalenz}

\begin{definition}[Erhaltene Operationen unter $\Phi_n$]
\label{def:erhaltene_operationen}
Eine binäre Operation $\circ$ auf einer algebraischen Struktur heißt \emph{erhalten unter der Abbildung $\Phi_n$}, wenn für alle Elemente $a, b$ der Struktur gilt:
\[
  \Phi_n(a \circ b) = \Phi_n(a) \star \Phi_n(b)
\]
wobei $\star$ die entsprechende Operation in der Zielstruktur ist.
\end{definition}

\begin{theorem}[Erhaltung der Matrixoperationen unter Graphdarstellung]
\label{thm:erhaltung_operationen}
Unter der kanonischen Abbildung $\Phi_n: \R^{n \times n} \to \{\text{gewichtete Digraphen mit } n \text{ Knoten}\}$ bleiben folgende Operationen erhalten:
\begin{enumerate}[label=(\roman*)]
  \item \textbf{Addition:} $\Phi_n(\vA + \vB) = \Phi_n(\vA) \oplus \Phi_n(\vB)$ (Überlagerung von Graphen mit Gewichtsaddition).
  \item \textbf{Skalarmultiplikation:} $\Phi_n(\alpha \cdot \vA) = \alpha \cdot \Phi_n(\vA)$ (gleichmäßige Gewichtsskalierung).
  \item \textbf{Matrizenmultiplikation:} $\Phi_n(\vA \cdot \vB) = \Phi_n(\vA) \circ \Phi_n(\vB)$ (Wegverkettung mit Gewichtsmultiplikation).
  \item \textbf{Transposition:} $\Phi_n(\vA^T)$ = $\Phi_n(\vA)$ mit umgekehrten Kantenrichtungen.
  \item \textbf{Determinante/Rang:} $\det(\vA) = 0 \iff \text{rank}(\Phi_n(\vA)) < n$ (Singularität in beiden Darstellungen äquivalent).
\end{enumerate}
\end{theorem}

\begin{proof}[Ausgewählte Operationen]
Wir zeigen exemplarisch (i) und (iii).

\textbf{(i) Addition:} Seien $\vA, \vB \in \R^{n \times n}$. Das Element $(\vA + \vB)_{ij} = \vA_{ij} + \vB_{ij}$.

Unter $\Phi_n$ sind die Kantengewichte $w_{\vA}(i,j) = \vA_{ij}$ und $w_{\vB}(i,j) = \vB_{ij}$.

Die Überlagerung $\Graph(\vA) \oplus \Graph(\vB)$ hat Kantengewichte $w_{\oplus}(i,j) = w_{\vA}(i,j) + w_{\vB}(i,j) = \vA_{ij} + \vB_{ij}$.

Dies ist genau $\Phi_n(\vA + \vB)$.

\textbf{(iii) Matrizenmultiplikation:} Sei $\vC = \vA \cdot \vB$. Dann:
\[
  \vC_{ij} = \sum_{k=1}^{n} \vA_{ik} \vB_{kj}
\]

Graphisch: Ein Weg von $i$ zu $j$ in $\Graph(\vA) \circ \Graph(\vB)$ ist eine Verkettung eines Weges von $i$ zu $k$ in $\Graph(\vA)$ mit Gewicht $\vA_{ik}$ und eines Weges von $k$ zu $j$ in $\Graph(\vB)$ mit Gewicht $\vB_{kj}$.

Das Gesamtgewicht ist $\vA_{ik} \cdot \vB_{kj}$, und die Summe über alle $k$ gibt genau $\vC_{ij}$.

Dies ist die Definition der Wegverkettungsoperation $\circ$ in der Graphik.
\end{proof}

\begin{corollary}[Algebraische Struktur ist invariant]
\label{cor:struktur_invariant}
Die algebraische Struktur $(\R^{n \times n}, +, \cdot, \times)$ (Addition, Skalarmultiplikation, Matrizenmultiplikation) ist isomorph zu der Struktur der Graphoperationen. Daher gibt es keinen algebraischen Grund, Matrizen anderen Graphen vorzuziehen.
\end{corollary}

\subsection{Spektrale Äquivalenz}
\label{subsec:spektrale_aequivalenz}

\begin{definition}[Spektrum eines Graphen]
\label{def:spektrum_graph}
Das \emph{Spektrum} eines Graphen $\Graph = (V, E, w)$ mit Adjazenzmatrix $\vA_{\Graph}$ ist die Menge der Eigenwerte von $\vA_{\Graph}$:
\[
  \mathrm{spec}(\Graph) = \{\lambda \in \C : \det(\vA_{\Graph} - \lambda \vI) = 0\}
\]
\end{definition}

\begin{theorem}[Spektrale Äquivalenz: Matrix $\leftrightarrow$ Graph]
\label{thm:spektrale_aequivalenz}
Für jede Matrix $\vA \in \R^{n \times n}$ und ihren zugehörigen Graphen $\Graph(\vA)$ gilt:
\[
  \mathrm{spec}(\vA) = \mathrm{spec}(\Graph(\vA))
\]

Das heißt, die Menge der Eigenwerte ist unter $\Phi_n$ invariant.
\end{theorem}

\begin{proof}
Die Adjazenzmatrix von $\Graph(\vA)$ ist definitionsgemäß $\vA_{\Graph(\vA)} = \vA$.

Daher:
\[
  \mathrm{spec}(\Graph(\vA)) = \{\lambda : \det(\vA_{\Graph(\vA)} - \lambda \vI) = 0\} = \{\lambda : \det(\vA - \lambda \vI) = 0\} = \mathrm{spec}(\vA)
\]
\end{proof}

\begin{corollary}[Alle spektralen Eigenschaften sind graphisch zugänglich]
\label{cor:spektrale_eigenschaften_graphisch}
Alle klassischen Sätze über Spektren von Matrizen (Spektralsatz, Eigenwertinequationen, Perturbationstheorie, etc.) haben unmittelbare graphische Entsprechungen. Daher sind Matrizen für spektrale Analysen überflüssig; Graphen liefern die gleiche Information.
\end{corollary}

\subsection{Struktur und Isomorphie}
\label{subsec:struktur_isomorphie}

\begin{definition}[Graph-Ähnlichkeit (Graphisches Äquivalent von Matrixähnlichkeit)]
\label{def:graph_aehnlichkeit}
Zwei Graphen $\Graph_1$ und $\Graph_2$ heißen \emph{ähnlich}, wenn es eine Bijektion $\pi$ der Knotenmenge gibt, die die Kantengewichte erhält:
\[
  w_1(u, v) = w_2(\pi(u), \pi(v)) \quad \text{für alle } u, v
\]
\end{definition}

\begin{theorem}[Ähnlichkeit unter Graphtransformation]
\label{thm:graph_aehnlichkeit_invarianz}
Zwei Matrizen $\vA$ und $\vB$ sind ähnlich (im klassischen Sinne: $\vB = \vP^{-1} \vA \vP$ für eine invertierbare Matrix $\vP$) genau dann, wenn die Graphen $\Graph(\vA)$ und $\Graph(\vB)$ ähnlich sind (graphisch: durch Relabeling der Knoten ineinander überführbar).
\end{theorem}

\begin{proof}
\textbf{($\Rightarrow$)} Angenommen, $\vB = \vP^{-1} \vA \vP$. Definiere $\pi$ durch die Permutation, die von $\vP$ induziert wird.

Für die Einträge von $\vB$ und $\vA$ gilt:
\[
  \vB_{\pi(i)\pi(j)} = \vA_{ij}
\]

Dies ist eine direkte Konsequenz der Matrixsimilaritäts-Definition.

Graphisch: $w_B(\pi(i), \pi(j)) = \vB_{\pi(i)\pi(j)} = \vA_{ij} = w_A(i, j)$.

Also sind $\Graph(\vA)$ und $\Graph(\vB)$ ähnlich.

\textbf{($\Leftarrow$)} Angenommen, $\Graph(\vA)$ und $\Graph(\vB)$ sind graphisch ähnlich unter einer Knotenpermutation $\pi$.

Definiere die Permutationsmatrix $\vP$ entsprechend zu $\pi$.
...
durch die Konstruktion der Ähnlichkeitsmatrix.
\end{proof}
...
% ============================================================
\section{Tiefere Analyse: Warum Induktion epistemisch fehlerhaft ist}
\label{sec:tiefere_induktion}
% ============================================================

\subsection{Die unendliche Quantifizierung als versteckte Annahme}
\label{subsec:unendliche_quantifizierung}

\begin{definition}[Induktive Aussage]
\label{def:induktive_aussage}
Eine \emph{induktive Aussage} ist eine universale Aussage der Form:
\[
  \forall n \in \N: P(n)
\]
wobei $P(n)$ ein Prädikat ist, das von einer natürlichen Zahl $n$ abhängt.
\end{definition}

\begin{lemma}[Struktur der Induktion]
\label{lem:struktur_induktion}
Um die induktive Aussage $\forall n \in \N: P(n)$ zu beweisen, werden zwei Dinge gezeigt:
\begin{enumerate}[label=(\alph*)]
  \item \emph{Basisfall:} $P(1)$ ist wahr (oder $P(0)$).
  \item \emph{Induktionsschritt:} Für alle $n$ gilt: $P(n) \implies P(n+1)$.
\end{enumerate}

Aus diesen folgt logisch $\forall n \in \N: P(n)$.
\end{lemma}

\begin{theorem}[Die versteckte Universalquantifizierung]
\label{thm:versteckte_quantifizierung}
Jeder induktive Beweis enthält eine \emph{versteckte universale Quantifizierung über alle natürlichen Zahlen}. Wenn die ursprüngliche Aufgabe nur eine Aussage über eine \emph{endliche} Menge $M \subset \N$ betrifft, dann beweist die Induktion zwangsläufig \emph{mehr} als verlangt.
\end{theorem}

\begin{proof}
Sei $M = \{m_1, m_2, \ldots, m_k\} \subset \N$ eine endliche Menge mit $m_1 < m_2 < \ldots < m_k$.

Die ursprüngliche Aufgabe: Beweise $P(m_i)$ für alle $m_i \in M$.

Das heißt, zeige: $P(m_1) \land P(m_2) \land \cdots \land P(m_k)$.

Ein induktiver Beweis zeigt stattdessen: $\forall n \in \N: P(n)$.

Dies ist äquivalent zu: $P(1) \land P(2) \land P(3) \land \cdots \land P(m_k) \land P(m_k+1) \land P(m_k+2) \land \cdots$.

Das bedeutet:
\begin{enumerate}[label=(\roman*)]
  \item Wir zeigen $P(m_i)$ für alle $m_i \in M$ (erfüllte Aufgabe)
  \item Wir zeigen zusätzlich $P(n)$ für alle unendlich vielen $n \notin M$ (überschüssig)
\end{enumerate}

Die induktive Methode \emph{quantifiziert implizit} über alle natürlichen Zahlen, nicht nur über die Elemente von $M$. Dies ist eine versteckte, unnötige Verallgemeinerung.

Daher zeigt die Induktion zu viel.
\end{proof}

\subsection{Warum Deduktion exakt ist}
\label{subsec:warum_deduktion_exakt}

\begin{definition}[Deduktive Exaktheit]
\label{def:deduktive_exaktheit}
Ein Beweis ist \emph{deduktiv exakt}, wenn jeder Beweisschritt direkt aus den gegebenen Annahmen und Axiomen folgt, ohne versteckte Quantifizierungen oder Verallgemeinerungen einzuführen.
\end{definition}

\begin{theorem}[Deduktion vermeidet Überverallgemeinerung]
\label{thm:deduktion_vermeidet}
Für jede endliche Struktur $S$ und jede Aussage $P$ über $S$ gibt es einen deduktiven Beweis, der:
\begin{enumerate}[label=(\alph*)]
  \item Genau und nur die Elemente von $S$ betrachtet.
  \item Keine externe Quantifizierung einführt.
  \item Exakt das zeigt, was zu zeigen ist.
\end{enumerate}
\end{theorem}

\begin{proof}
Sei $S$ eine endliche Struktur mit Elementen $s_1, \ldots, s_k$.

Ein deduktiver Beweis für $P$ operiert direkt auf diesen Elementen:
\begin{enumerate}[label=(\roman*)]
  \item Sind die Elemente endlich?
\end{enumerate}

Ja. Dann kann die Aussage $P(S)$ in eine endliche Konjunktion von Aussagen zerlegt werden:
\[
  P(S) \equiv \bigwedge_{i=1}^{k} Q_i(s_i)
\]

wobei $Q_i(s_i)$ eine Aussage über ein einzelnes Element $s_i$ ist.

Beweise dann $Q_1(s_1), Q_2(s_2), \ldots, Q_k(s_k)$ einzeln und deduziere $P(S)$ durch Konjunktion.

Jeder Schritt bezieht sich auf ein konkretes Element und keine versteckte Quantifizierung.

Dies ist der Beweis für $P(S)$, nicht für etwas Größeres.
\end{proof}

\subsection{Das Problem der Induktion im Kontext von Graphen}
\label{subsec:induktion_graphen}

\begin{example}[Pfadanzahl in einem Graphen]
\label{ex:pfadanzahl}
\emph{Aufgabe:} In einem gegebenen Graphen $\Graph$ mit $n=5$ Knoten: Wie viele Wege der Länge 3 gibt es von Knoten $a$ zu Knoten $b$?

\emph{Induktive Methode (fehlerhaft):} 
Man könnte versuchen, durch Induktion über die Weglänge $\ell$ zu beweisen, dass die Formel für alle $\ell \in \N$ gilt. Aber die Aufgabe betrifft nur $\ell = 3$! 

Der induktive Beweis würde Formeln für alle Weglängen $1, 2, 3, 4, 5, \ldots$ herleiten -- weit mehr als gefragt.

\emph{Deduktive Methode (korrekt):}
Enumeriere direkt alle Wege der Länge 3 im Graphen. Dies dauert möglicherweise länger, aber es beweist genau, was verlangt wird, ohne Überverallgemeinerung.
\end{example}

\begin{theorem}[Induktion ist für Graphen strukturell unangemessen]
\label{thm:induktion_graphen_unangemessen}
Bei endlichen Graphen $\Graph = (V, E)$ mit $|V| = n < \infty$ ist jeder Beweis durch Induktion über $|V|$ oder die Kantenzahl methodisch fehlerhaft. Der Grund: Die Induktion verallgemeinert auf Graphen mit unendlich vielen Knoten, obwohl nur endliche Graphen gegeben sind.
\end{theorem}

\begin{proof}
Ein Induktionsbeweis für eine Aussage $P$ über einen Graphen mit $n$ Knoten hätte folgende Struktur:

\begin{enumerate}[label=(\roman*)]
  \item \emph{Basisfall:} Zeige $P$ für Graphen mit 1 Knoten.
  \item \emph{Schritt:} Zeige: Wenn $P$ für Graphen mit $n$ Knoten wahr ist, dann auch für $n+1$ Knoten.
  \item \emph{Abschluss:} Daher ist $P$ für alle $n \in \N$ Knoten wahr.
\end{enumerate}

Aber die ursprüngliche Aufgabe war: Beweise $P$ für einen Graphen mit \emph{genau} $n=5$ Knoten (oder einer anderen festen Zahl).

Der induktive Beweis zeigt aber $P$ für alle Graphen mit 1, 2, 3, ..., unendlich vielen Knoten.

Das ist ein gravierendes Overproof: Wir beweisen Wahrheiten über unendliche Strukturen, während nur eine endliche gegeben ist.

Ein deduktiver Beweis würde stattdessen direkt den Graphen mit 5 Knoten untersuchen und $P$ für eben diesen zeigen.
\end{proof}
...
\begin{remark}[Warum dieser Beweis nicht induktiv sein muss]
\label{rem:nicht_induktiv}
Beachte: Dieser Beweis nutzt \emph{keine} Induktion. Er funktioniert direkt durch Wegverkettung, unabhängig davon, wie lang die Wege sind. Die Länge der Wege ist für die Gültigkeit der Transitivität irrelevant. Ein induktiver Beweis (über die Weglänge) würde unnötiger Weise zeigen, dass die Transitivität für alle natürlichen Zahlenlängen gilt, obwohl nur konkrete Wege relevant sind.
\end{remark}
...
\subsection{Beispiel 4: Zusammenhang und Zyklenfreiheit}
\label{subsec:beispiel_zusammenhang}

\begin{definition}[Zusammenhang und Zyklen in Graphen]
\label{def:zusammenhang_zyklen}
Ein Graph $\Graph = (V, E)$ heißt \emph{zusammenhängend}, wenn jeder Knoten von jedem anderen Knoten erreichbar ist. Ein \emph{Zyklus} ist ein Weg, der zum gleichen Knoten zurückkehrt.
\end{definition}

\begin{theorem}[Baumstruktur und Zusammenhang]
\label{thm:baum_zusammenhang}
Ein zusammenhängender Graph $\Graph = (V, E)$ mit $|V| = n$ Knoten ist genau dann zyklenfrei, wenn $|E| = n-1$.
\end{theorem}

\begin{proof}[Vollständig deduktiver Beweis]
Wir zeigen beide Richtungen.

\textbf{($\Rightarrow$)} Angenommen, $\Graph$ ist zusammenhängend und zyklenfrei.

Wir führen einen deduktiven Beweis durch strukturelle Analyse:

\begin{enumerate}[label=(\roman*)]
  \item Da $\Graph$ zusammenhängend ist, existiert für jeden Knoten $v \in V$ ein Weg von einem Referenzknoten $r$ zu $v$.
  
  \item Betrachte einen Spannbaum $T$ von $\Graph$ (einen maximal zusammenhängenden zyklenfrei Teilgraph). Ein Spannbaum mit $n$ Knoten hat immer $n-1$ Kanten -- dies ist eine direkte Konsequenz der Definition.
  
  \item Da $\Graph$ zyklenfrei ist, können keine Kanten außerhalb von $T$ hinzugefügt werden, ohne einen Zyklus zu erzeugen (by contrapositive der Zyklendefinition).
  
  \item Also ist $\Graph = T$, und somit $|E| = n - 1$.
\end{enumerate}

\textbf{($\Leftarrow$)} Angenommen, $\Graph$ ist zusammenhängend mit $|E| = n-1$.

\begin{enumerate}[label=(\roman*)]
  \item Ein zusammenhängender Graph mit $n$ Knoten muss mindestens $n-1$ Kanten haben (erforderlich für Zusammenhang).
  
  \item Wir haben exakt $|E| = n-1$. 
  
  \item Angenommen, $\Graph$ enthält einen Zyklus $C$. Entferne eine beliebige Kante $(u, v)$ aus $C$. Der resultierende Graph $\Graph' = (V, E \setminus \{(u,v)\})$ bleibt zusammenhängend (weil der Zyklus alternative Wege zwischen $u$ und $v$ bietet).
  
  \item Aber dann hat $\Graph'$ weniger als $n-1$ Kanten und ist noch zusammenhängend -- ein Widerspruch zu (i).
  
  \item Also enthält $\Graph$ keinen Zyklus.
\end{enumerate}

Daher ist $\Graph$ genau dann zyklenfrei, wenn $|E| = n-1$.
\end{proof}

\begin{remark}[Kein induktiver Beweis nötig]
\label{rem:baum_keine_induktion}
Dieser Beweis funktioniert vollständig ohne Induktion. Er operiert direkt auf den Begriffen "zusammenhängend", "zyklenfrei" und "Kantenzahl" für einen konkreten endlichen Graphen. Eine induktive Formulierung (z.B. "Induktion über die Anzahl der Knoten") wäre methodisch redundant und würde unnötig über alle möglichen Graphgrößen quantifizieren.
\end{remark}

\subsection{Beispiel 5: Starke Zusammenhang und Zyklen in gerichteten Graphen}
\label{subsec:beispiel_stark_zusammenhang}

\begin{definition}[Stark zusammenhängend]
\label{def:stark_zusammenhaengend}
Ein gerichteter Graph $\Graph = (V, E)$ heißt \emph{stark zusammenhängend}, wenn für alle zwei Knoten $u, v \in V$ ein gerichteter Weg von $u$ zu $v$ existiert.
\end{definition}

\begin{theorem}[Charakterisierung stark zusammenhängender Graphen]
\label{thm:scc_charakterisierung}
Ein gerichteter Graph $\Graph = (V, E)$ ist genau dann stark zusammenhängend, wenn es einen geschlossenen Weg (Zyklus) gibt, der alle Knoten besucht.
\end{theorem}

\begin{proof}[Direkt deduktiver Beweis]
\textbf{($\Rightarrow$)} Angenommen, $\Graph$ ist stark zusammenhängend.

Wähle einen beliebigen Knoten $v_1 \in V$. 

Für jeden weiteren Knoten $v_i \in V \setminus \{v_1\}$ existiert ein Weg von $v_1$ zu $v_i$ (nach Definition). Verkette diese Wege in beliebiger Reihenfolge:
\[
  W = v_1 \to \cdots \to v_2 \to \cdots \to v_3 \to \cdots \to v_n \to \cdots \to v_1
\]

Der letzte Schritt $v_n \to v_1$ existiert, weil $\Graph$ stark zusammenhängend ist (Weg von $v_n$ zu $v_1$).

Dieser Weg $W$ besucht alle Knoten und ist geschlossen (beginnt und endet bei $v_1$).

\textbf{($\Leftarrow$)} Angenommen, es existiert ein geschlossener Weg $W$, der alle Knoten besucht.

Sei $u, v \in V$ zwei beliebige Knoten. Da $W$ alle Knoten besucht, treten sowohl $u$ als auch $v$ in $W$ auf.

Folge dem Weg $W$ von $u$ bis $v$ -- dies gibt einen Weg von $u$ zu $v$.

Also ist jeder Knoten von jedem anderen erreichbar, d.h., $\Graph$ ist stark zusammenhängend.
\end{proof}

\subsection{Beispiel 6: Kürzeste Wege und graphische Optimierung}
\label{subsec:beispiel_kuerzeste_wege}

\begin{definition}[Gewichteter Weg]
\label{def:gewichteter_weg}
Ein \emph{gewichteter Weg} in einem gewichteten Graphen $\Graph = (V, E, w)$ ist eine Sequenz von Knoten $(v_0, v_1, \ldots, v_k)$ mit Gesamtgewicht $W = \sum_{i=0}^{k-1} w(v_i, v_{i+1})$.
\end{definition}

\begin{theorem}[Existenz kürzester Wege in endlichen Graphen]
\label{thm:kuerzeste_wege}
In einem endlichen gewichteten gerichteten Graphen $\Graph = (V, E, w)$ mit $|V| = n$ existiert für jedes Knotenpaar $(u, v)$ entweder:
\begin{enumerate}[label=(\alph*)]
  \item Ein kürzester Weg mit endlichem Gewicht, oder
  \item Ein negativer Zyklus auf dem Weg von $u$ zu $v$ (wenn Gewichte negativ sein können).
\end{enumerate}
\end{theorem}

\begin{proof}[Deduktiver konstruktiver Beweis]
Gegeben sind zwei Knoten $u, v \in V$.

\textbf{Fall 1:} Es existiert kein Weg von $u$ zu $v$. Dann gibt es keinen kürzesten Weg (oder das Gewicht ist $\infty$).

\textbf{Fall 2:} Es existiert mindestens ein Weg von $u$ zu $v$.

Betrachte alle möglichen Wege von $u$ zu $v$. Da der Graph endlich ist ($|V| = n$), gibt es endlich viele Kanten in $E$. 

Ein Weg ohne Zyklen kann maximal $n-1$ Kanten enthalten (ein \emph{einfacher Weg}). Es gibt $\binom{n}{2}$ mögliche einfache Wege (im Worst Case).

\textbf{Fall 2a:} Alle einfachen Wege von $u$ zu $v$ haben endliche Gewichte. Der kürzeste unter ihnen ist der \emph{kürzeste Weg}. Dieser existiert und ist minimal.

\textbf{Fall 2b:} Es existiert ein Weg mit einem Zyklus negativen Gewichts. Wenn wir diesen Zyklus mehrfach durchlaufen, sinkt das Gesamtgewicht ins Unendliche. In diesem Fall existiert kein Minimum (aber ein negativer Zyklus).

In jedem Fall ist die Situation für den endlichen Graphen klar definiert: Entweder existiert ein kürzester Weg, oder es gibt einen negativen Zyklus.

Kein drittes Szenario ist möglich.
\end{proof}

\begin{remark}[Endlichkeit eliminiert unklare Fälle]
\label{rem:endlichkeit_klar}
Dieser Beweis zeigt, dass für endliche Graphen alle Fälle durch direktes Aufzählen und Analyse klar werden. Keine Induktion ist notwendig -- die Endlichkeit des Graphen selbst garantiert, dass wir alle Wege untersuchen können. In unendlichen Graphen würde die Situation komplizierter (und möglicherweise induktive oder limitative Argumente wären erforderlich).
\end{remark}

% ============================================================
\section{Fallstudien: Konkrete Vergleiche zwischen Matrix- und Graphendarstellung}
\label{sec:fallstudien}
% ============================================================

\subsection{Fallstudie 1: Erreichbarkeitsanalyse in Netzwerken}
\label{subsec:fallstudie_erreichbarkeit}

\begin{example}[Das Erreichbarkeitsproblem]
\label{ex:erreichbarkeitsproblem}
\emph{Aufgabe:} In einem sozialen Netzwerk mit 5 Personen (A, B, C, D, E) und bekannten Verbindungen (Freundschaften) ist die Frage: Wer ist von Person A aus erreichbar (direkt oder indirekt)?
...
Die Erreichbarkeit wird durch die Berechnung von $\vA^k$ (k-te Potenzen) bestimmt. Wenn $(\vA^k)_{ij} > 0$, dann gibt es einen Weg von $i$ zu $j$ der Länge $k$.

Um alle Wege zu erfassen, müsste man potenziell bis $\vA^{n-1}$ rechnen (schlimmster Fall für einen einfachen Weg ohne Zyklen).

\emph{Graphdarstellung:}
Zeichne die 5 Knoten und die Kanten:
\[
\begin{tikzpicture}[node distance=2cm]
  \node (A) [circle, draw] {A};
  \node (B) [circle, draw, right of=A] {B};
  \node (C) [circle, draw, right of=B] {C};
  \node (D) [circle, draw, right of=C] {D};
...
Erreichbarkeit ist unmittelbar visuell klar: Folge den Kanten von A aus. Man sieht sofort, dass A alle anderen Knoten erreichen kann (eine lineare Struktur).

\emph{Deduktiver Beweis (graphisch):}
\begin{enumerate}[label=(\alph*)]
  \item Von A zu B: direkte Kante A-B existiert.
  \item Von A zu C: Weg A-B-C existiert (nutze die Transitivität der Erreichbarkeit).
  \item Von A zu D: Weg A-B-C-D existiert.
  \item Von A zu E: Weg A-B-C-D-E existiert.
\end{enumerate}

Alle Knoten sind von A erreichbar.

\emph{Vergleich:}
Die Graphik ist intuitiver und direkter. Es ist klar, dass dies eine lineare Struktur ist. Die Matrixrechnung ist mechanischer und weniger informativ.
\end{example}

\subsection{Fallstudie 2: Zyklenerkennung und -vermeidung}
\label{subsec:fallstudie_zyklen}

\begin{example}[Das Zyklenproblem]
\label{ex:zyklenproblem}
\emph{Aufgabe:} Bestimme, ob ein gegebener gerichteter Graph einen Zyklus enthält.

\emph{Matrixdarstellung:}
Eine Methode: Berechne $\text{tr}(\vA^k)$ (Spur) für $k = 1, 2, 3, \ldots$. Wenn eine Spur größer als 0 ist, gibt es einen Zyklus der Länge $k$.

Aber: Die Spur ist ein globales Konzept. Um zu verstehen, welche Zyklen existieren, muss man interpretieren.

\emph{Graphdarstellung:}
Visualisiere die Kanten. Suche visuell oder algorithmisch (z.B. Tiefensuche) nach Zyklen.

Graphisch: Sind dort Pfade, die zu sich selbst zurückkehren? Sofort sichtbar in einer Zeichnung.

\emph{Deduktiver Beweis (graphisch):}
Ein Zyklus ist eine Sequenz von Knoten $(v_0, v_1, \ldots, v_k, v_0)$ mit Kanten $(v_i, v_{i+1})$ für alle $i$.

Durchsuche den Graphen mittels Tiefensuche (DFS):
\begin{enumerate}[label=(\alph*)]
  \item Starte bei einem Knoten $v$.
  \item Wenn du eine bereits besuchte Kante (Rückwärtskante) triffst, gibt es einen Zyklus.
  \item Wenn die Tiefensuche beendet ist, ohne eine Rückwärtskante zu finden, gibt es keinen Zyklus.
\end{enumerate}

Dieser Algorithmus ist direkter als Matrixpotenzen.

\emph{Vergleich:}
Die graphische Methode ist algorithmisch und visuell klarer. Sie separiert die Suche nach Zyklen von abstrakten Matrixeigenschaften.
\end{example}

\subsection{Fallstudie 3: Spektrale Eigenschaften und dynamische Systeme}
\label{subsec:fallstudie_spektral}

\begin{example}[Dynamische Stabilität]
\label{ex:dynamische_stabilitaet}
\emph{Aufgabe:} Ist das dynamische System $\dot{\mathbf{x}}(t) = \vA \mathbf{x}(t)$ stabil (d.h., konvergiert $\mathbf{x}(t) \to 0$)?

\emph{Matrixdarstellung:}
Berechne die Eigenwerte $\lambda_1, \ldots, \lambda_n$ der Matrix $\vA$. Das System ist stabil, wenn alle Eigenwerte negative Realteile haben: $\text{Re}(\lambda_i) < 0$.

Dies erfordert numerische Berechnung oder charakteristisches Polynom.

\emph{Graphdarstellung:}
Der Graph $\Graph(\vA)$ kodiert die Dynamik als Gewichtsverteilung auf Kanten. Ein Eigenwert $\lambda$ mit Eigenvektor $\mathbf{v}$ entspricht einer Eigenmode, bei der die Gewichtsverteilung $\mathbf{v}$ sich um den Faktor $e^{\lambda t}$ entlang den Kanten propagiert.

Graphisch verstehen: Wenn alle Eigenweigths $\lambda_i$ negativ sind, klingt jede Gewichtsverteilung ab. Dies ist eine Aussage über die spektralen Eigenschaften des Graphen.

\emph{Graphische Intuition:}
Ein Knoten mit Gewicht verdampft entlang der Kanten. Wenn die Kanten strukturiert sind (z.B. symmetrisch, schwach gekoppelt), kann das Gewicht erhalten bleiben (instabil). Wenn die Kanten strukturiert sind (z.B. dissipativ), verdampft es (stabil).

\emph{Vergleich:}
Die Matrixformulierung ist abstrakt. Die graphische Intuition ist physischer und geometrischer. Für komplexe Systeme gibt die graphische Ansicht oft schneller Einsicht in das Langzeitverhalten.
\end{example}

\subsection{Fallstudie 4: PageRank und Ranking-Algorithmen}
\label{subsec:fallstudie_pagerank}

\begin{example}[PageRank-Berechnung]
\label{ex:pagerank}
\emph{Aufgabe:} Berechne die Wichtigkeit (Ranking) aller Webseiten in einem Hyperlink-Netzwerk.

\emph{Matrixdarstellung:}
Die Übergangsmatrix $\vP$ ist gegeben durch:
\[
\vP_{ij} = \frac{\text{(Anzahl Links von Seite } i \text{ zu } j \text{)}}{\text{(Gesamtzahl Links von Seite } i \text{)}}
\]

Der PageRank-Vektor $\mathbf{r}$ erfüllt:
\[
\mathbf{r} = \vP^T \mathbf{r}
\]

Dies ist ein Eigenvektorproblem. Man löst es iterativ durch:
\[
\mathbf{r}^{(k+1)} = \vP^T \mathbf{r}^{(k)}
\]

bis Konvergenz.

\emph{Graphendarstellung:}
Der Graph hat Webseiten als Knoten und Hyperlinks als gerichtete Gewichtete Kanten. Das Gewicht ist die Wahrscheinlichkeit, von einer Seite zu einer anderen zu navigieren.

Der PageRank ist die stationäre Verteilung eines Random Walk auf diesem Graphen.

Graphisch: Stelle dir einen zufälligen Wanderer vor, der auf den Kanten des Graphen umhergeht. Die Häufigkeit, mit der er auf einem bestimmten Knoten ankommt, ist der PageRank dieses Knotens.

\emph{Graphische Intuition:}
Knoten mit vielen eingehenden Kanten (von wichtigen Knoten) sind selbst wichtig. Die Wichtigkeit fliesst wie Wasser durch die Kanten.

\emph{Vergleich:}
Beide Formulierungen sind äquivalent. Aber die graphische Intuition des Random Walk ist leichter zu verstehen als die abstrakte Eigenvektorgleichung. Computersysteme können beide nutzen; konzeptionell ist die Grafik klarer.
\end{example}

\subsection{Zusammenfassung der Fallstudien}
\label{subsec:fallstudien_zusammenfassung}

\begin{theorem}[Praktische Äquivalenz der Darstellungen]
\label{thm:praktische_aequivalenz}
In allen vier Fallstudien zeigt sich:
\begin{enumerate}[label=(\alph*)]
  \item Die Matrix- und Graphendarstellung liefern \emph{identische mathematische Ergebnisse}.
  \item Die Graphendarstellung ist oft konzeptionell klarer und intuitiver.
  \item Die Graphendarstellung ermöglicht direktere deduktive Beweise.
  \item Für praktische Probleme kann die Graphendarstellung häufig effizienter sein (visuell, algorithmisch).
\end{enumerate}

Daher gibt es keine mathematischen oder praktischen Gründe, Matrizen als Primitive zu verwenden, wenn Graphen verfügbar sind.
\end{theorem}

% ============================================================
\section{Rechnerische Aspekte und Algorithmen}
\label{sec:algorithmen}
% ============================================================

\subsection{Graphische Algorithmen als Matrixalgorithmen}
\label{subsec:graphische_algorithmen}

\begin{definition}[Graphische Tiefensuche (DFS)]
\label{def:dfs}
Die \emph{Tiefensuche} auf einem Graphen $\Graph = (V, E)$ ist ein Algorithmus, der systematisch alle erreichbaren Knoten von einem Startknoten besucht, indem er in die Tiefe folgt (einen Ast des Graphen vollständig erforscht, bevor er zum nächsten übergeht).
\end{definition}

\begin{theorem}[DFS als deduktive Strukturanalyse]
\label{thm:dfs_deduktiv}
Der DFS-Algorithmus ist ein deduktives Verfahren (kein induktives), da er:
\begin{enumerate}[label=(\alph*)]
  \item Alle besuchten Knoten aufzählt (endlich viele, da der Graph endlich ist).
  \item Für jeden Knoten die Kanten prüft (endlich viele pro Knoten).
  \item Nach einem endlichen Durchlauf eine vollständige Antwort gibt (alle erreichbaren Knoten).
\end{enumerate}

Es gibt keine Quantifizierung über unendliche Sequenzen; DFS ist exakt und lokal.
\end{theorem}

\begin{example}[DFS-Anwendung: Zykluserkennung]
\label{ex:dfs_zyklen}
Ein Graph mit 4 Knoten und Kanten $(1,2), (2,3), (3,1), (3,4)$:
...
\begin{tikzpicture}[node distance=2cm, auto]
  ...
  \node (4) [circle, draw, below of=3] {4};
  
  \path[draw, ->, thick] (1) -- (2) -- (3) -- (1);
  \path[draw, ->, thick] (3) -- (4);
\end{tikzpicture}
\]

DFS-Durchlauf von Knoten 1:
\begin{enumerate}[label=(\alph*)]
  \item Besuche 1 (Start).
  \item Folge Kante zu 2, besuche 2.
  \item Folge Kante zu 3, besuche 3.
  \item Folge Kante zu 1 (bereits besucht!) -- \textbf{Zyklus erkannt} $(1 \to 2 \to 3 \to 1)$.
  \item Folge Kante zu 4, besuche 4.
  \item Keine weiteren Kanten.
\end{enumerate}

\textbf{Ergebnis:} Ein Zyklus der Länge 3 existiert. Dieser Beweis ist vollständig deduktiv und erfordert keine Induktion.
\end{example}

\subsection{Breitensuche und Kürzeste-Weg-Algorithmen}
\label{subsec:breitensuche}

\begin{definition}[Graphische Breitensuche (BFS)]
\label{def:bfs}
Die \emph{Breitensuche} auf einem Graphen $\Graph = (V, E)$ ist ein Algorithmus, der alle Knoten in Ebenen untersucht: Zuerst der Startknoten (Ebene 0), dann alle Nachbarn (Ebene 1), dann deren Nachbarn (Ebene 2), usw.
\end{definition}

\begin{theorem}[BFS findet kürzeste Wege in ungewichteten Graphen]
\label{thm:bfs_kuerzeste_wege}
Für einen ungewichteten Graphen $\Graph = (V, E)$ und zwei Knoten $u, v \in V$ findet der BFS-Algorithmus den kürzesten Weg (gemessen in Kantenzahl) von $u$ zu $v$ in endlicher Zeit.
\end{theorem}

\begin{proof}[Deduktiver Beweis der Korrektheit von BFS]
\begin{enumerate}[label=(\roman*)]
  \item BFS organisiert Knoten in Ebenen $L_0, L_1, L_2, \ldots$, wobei $L_k$ alle Knoten sind, die in genau $k$ Schritten von $u$ erreicht werden.
  
  \item Für jeden Knoten $v \in L_k$ gibt es einen Weg von $u$ zu $v$ mit $k$ Kanten (per Definition von $L_k$).
  
  \item Wenn es einen kürzeren Weg gäbe (weniger als $k$ Kanten), würde $v$ in einer früheren Ebene $L_{k'}, k' < k$ sein -- Widerspruch.
  
  \item Also ist der von BFS gefundene Weg minimal.
  
  \item Seit der Graph endlich ist, gibt es nur endlich viele Ebenen ($\leq n-1$ für $n$ Knoten), so dass BFS in endlicher Zeit terminiert.
\end{enumerate}

Dies ist ein deduktiver Beweis: keine Quantifizierung über unendliche Strukturen.
\end{proof}

\subsection{Vergleich: Deduktive Algorithmen vs. Induktive Beweise}
\label{subsec:vergleich_algo_induktion}

\begin{table}[H]
\centering
\caption{Charakteristiken deduktiver Algorithmen vs. induktiver Beweise}
\label{tab:algo_vs_induktion}
\begin{tabular}{p{3cm}|p{3.5cm}|p{3.5cm}}
\toprule
\textbf{Merkmal} & \textbf{Deduktive Algorithmen} & \textbf{Induktive Beweise} \\
\midrule
Struktur & Konkrete Schritte auf einem Objekt & Schema (Basis + Schritt) \\
Quantifizierung & Über endliche Elemente & Über alle natürlichen Zahlen \\
Abschlusszeit & Endlich (für endliche Eingaben) & Unbegrenzt notional \\
Visualisierbar & Ja (schrittweise Abarbeitung) & Nein (konzeptionell abstrakt) \\
Implementierbar & Direkt als Code & Erfordert Umsetzung \\
Überproof? & Nein (genau das Nötige) & Ja (zu viel gezeigt) \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Praktische Konsequenz: Graphische Programme}
\label{subsec:praktische_konsequenz}

\begin{remark}[Programmierbare Graphen]
\label{rem:programmierbare_graphen}
Das stärkste Argument für Graphen über Matrizen ist die \emph{programmierbare Transparenz}:

Ein Graphalgorithmus ist ein endliches Programm mit endlichen Schleifen und bedingten Anweisungen. Jeder Schritt ist überprüfbar, nachvollziehbar und deduktiv.

Ein induktiver Beweis hingegen verlässt sich auf ein abstraktes Schema, das über die Unendlichkeit "springt". Dies ist konzeptionell, nicht algorithmisch.

Für die moderne Informatik und computergestützte Verifikation sind graphische (algorithmic) Formulierungen daher zu bevorzugen.
\end{remark}
...
\subsection{Fragen zur zukünftigen Forschung}
\label{subsec:zukuenftige_forschung}

\begin{enumerate}[label=(\roman*)]
  \item \textbf{Sind alle Theoreme in deduktiver Form beweisbar?} Oder gibt es Sätze, für die induktive Methoden (scheinbar) notwendig sind? Wenn ja: Warum? Können diese umformuliert werden?
  
  \item \textbf{Kann die graphische Notation vollständig standardisiert werden?} Für die mathematische Praxis braucht man eine ebenso prägnante Notation wie für Matrizen.
  
  \item \textbf{Wie behandelt man unendliche Graphen?} Müssen neue Konzepte entwickelt werden für überabzählbare oder kontinuierliche Strukturen?
  
  \item \textbf{Kann maschinelles Theorem-Proving graphisch realisiert werden?} Würde es schneller oder langsamer als Matrix-Beweise?
  
  \item \textbf{Gibt es Probleme, die in Graphform schwieriger werden?} Oder sind sie nur eine Umformulierung?
  
  \item \textbf{Wie können physikalische Systeme (Hamiltonsche Mechanik, Quantenmechanik) vollständig graphisch formuliert werden?}
  
  \item \textbf{Induktion in unendlichen Kontexten:} Wenn unendliche Graphen relevant sind, braucht man eventuell Induktion. Kann man zwischen "legitimer" Induktion (für unendliche Objekte) und "illegitimer" Induktion (für endliche Objekte) unterscheiden?
\end{enumerate}

\subsection{Beziehung zur Kategorientheorie}
\label{subsec:kategorientheorie}

\begin{remark}[Kategorisches Fundament]
\label{rem:kategorisches_fundament}
Die vorliegende Arbeit kann in kategorientheoretischen Begriffen reformuliert werden:

\begin{enumerate}[label=(\alph*)]
  \item Die Kategorie der $n \times n$ Matrizen ist isomorph zur Kategorie der gewichteten gerichteten Graphen mit $n$ Knoten.
  
  \item Die kanonische Abbildung $\Phi_n$ ist ein Funktor zwischen diesen Kategorien.
  
  \item Die Äquivalenz der Darstellungen ist eine \emph{natürliche Isomorphie}.
  
  \item Dies eröffnet tiefere Verbindungen zu höherkategorialen Strukturen.
\end{enumerate}

Eine vollständige kategorientheoretische Behandlung kann in zukünftigen Arbeiten erfolgen.
\end{remark}
...
\section{Anhang: Erweiterte mathematische Resultate}
\label{sec:anhang}

\subsection{Detaillierter Beweis der Eindeutigkeit des Minimal-Graphen}
\label{subsec:anhang_eindeutigkeit}

\begin{lemma}[Nicht-triviale Graphen müssen mindestens eine Kante haben]
\label{lem:anhang_nicht_trivial}
Ein Graph $\Graph = (V, E)$ ist \emph{nicht-trivial} (d.h., strukturell interessant), wenn $|V| \geq 2$ und $|E| \geq 1$.
\end{lemma}

\begin{proof}
Ein Graph mit $|V| = 1$ hat nur einen Knoten. Eine Kante würde nur eine Schleife sein. Dies ist nicht interessant für relationale Strukturen (ein Element bezieht sich auf sich selbst).

Ein Graph mit $|V| \geq 2$ und $|E| = 0$ hat nur isolierte Knoten. Dies ist auch nicht interessant für Zusammenhang.

Also müssen beide Bedingungen erfüllt sein.
\end{proof}

\begin{lemma}[Minimalität impliziert $|V| = 2, |E| = 1$]
\label{lem:anhang_minimalitaet}
Wenn $\Graph = (V, E)$ minimal-nichtleer ist, dann $|V| = 2$ und $|E| = 1$.
\end{lemma}

\begin{proof}
Aus der Definition der Minimalität (Definition~\ref{def:minimalitaet}) folgt:

\begin{enumerate}[label=(\roman*)]
  \item Der Graph ist nichtleer und zusammenhängend.
  
  \item Entfernung einer beliebigen Kante führt zu Unzusammenhang.
  
  Dies bedeutet, dass jede Kante ein \emph{Brückenkante} (Bridge) ist.
\end{enumerate}

Ein Graph mit nur Brückenkanten (und zusammenhängend) ist ein Baum. Ein Baum mit $n$ Knoten hat $n-1$ Kanten.

Wenn der Baum minimal ist, muss er die kleinste mögliche Größe haben: $n = 2$, also $|E| = 1$.

Dies ist der Zwei-Knoten-Graph.
\end{proof}

\subsection{Beweis der strukturellen Äquivalenz}
\label{subsec:anhang_struktur}

\begin{theorem}[Vollständiger Beweis der strukturellen Äquivalenz (erweitert)]
\label{thm:anhang_struktur_aequivalenz}
Alle algebraischen Eigenschaften von Matrizen haben graphische Entsprechungen.
\end{theorem}

\begin{proof}
Wir listen die wichtigsten Eigenschaften auf:

\begin{enumerate}[label=(\alph*)]
  \item \textbf{Rang:} Der Rang einer Matrix ist die Dimension des Bildes. Graphisch ist dies die Anzahl der erreichbaren Knoten (oder die Dimension des Graphencode-Raums).
  
  \item \textbf{Determinante:} Die Determinante misst das Volumen der Linearen Transformation. Graphisch ist dies ein Maß für die Gewichtsumwandlung unter graphischer Komposition.
  
  \item \textbf{Eigenwerte:} Eigenwerte sind Skalierungsfaktoren unter Transformation. Graphisch sind sie Faktoren, um die sich Gewichtsverteilungen unter Graphkomposition skalieren.
  
  \item \textbf{Spur:} Die Spur ist die Summe der Diagonalelemente. Graphisch ist dies die Summe der Gewichte der Selbstschleifen.
  
  \item \textbf{Ähnlichkeit:} Zwei Matrizen sind ähnlich, wenn sie durch Basiswechsel auseinander hervorgehen. Graphisch sind zwei Graphen ähnlich, wenn einer durch Knotenumetikettierung aus dem anderen hervorgeht.
  
  \item \textbf{Orthogonalität:} Orthogonale Matrizen erhalten Längen. Graphisch entspricht dies Isometrien der Kantengewichte unter Komposition.
\end{enumerate}

In jedem Fall gibt es eine bijektive Entsprechung.
\end{proof}

\subsection{Induktions-Fallstricke: Detaillierte Beispiele}
\label{subsec:anhang_induktion_fallstricke}

\begin{example}[Induktion erzeugt Übergeneral­isierungen]
\label{ex:anhang_induktion_probleme}
\emph{Aufgabe:} Beweise, dass ein spezifischer Graph mit $n=4$ Knoten zusammenhängend ist.

\emph{Induktiver Ansatz (fehlerhaft):}
\begin{enumerate}[label=(\roman*)]
  \item Basis: Zeige, dass ein 1-Knoten-Graph zusammenhängend ist. (Trivial, wahr.)
  
  \item Schritt: Zeige, dass wenn ein $n$-Knoten-Graph zusammenhängend ist, auch jeder $(n+1)$-Knoten-Graph zusammenhängend ist. (Dies ist \emph{nicht wahr} und kann nicht gezeigt werden!)
  
  \item Abschluss: (Zusammenbruch -- die Induktion funktioniert nicht.)
\end{enumerate}

\textbf{Warum ist dieser Ansatz problematisch?}

Der Induktionsschritt versucht, alle möglichen Graphen mit $n+1$ Knoten zu behandeln, nicht nur den gegebenen. Dies ist ein Overproof und methodisch unangemessen.

\emph{Deduktiver Ansatz (korrekt):}

Untersuche den spezifischen 4-Knoten-Graphen direkt. Prüfe, ob von jedem Knoten zu jedem anderen Knoten ein Weg existiert. Dies dauert möglicherweise länger, zeigt aber genau das, was verlangt wird.
\end{example}

\subsection{Vollständige Äquivalenzbeweis zwischen Graphen und Matrizen}
\label{subsec:anhang_vollständige_aequivalenz}

\begin{theorem}[Vollständige kategorische Äquivalenz]
\label{thm:anhang_kategorische_aequivalenz}
Die Kategorie $\mathbf{Mat}_n$ der $n \times n$ reellen Matrizen mit Matrizenmultiplikation als Morphismen ist äquivalent zu der Kategorie $\mathbf{Gra}_n$ der gewichteten gerichteten Graphen mit $n$ Knoten und graphischer Komposition als Morphismen.
\end{theorem}

\begin{proof}[Kategorische Äquivalenz]
Ein Äquivalenzfunktor ist gegeben durch $\Phi_n: \mathbf{Mat}_n \to \mathbf{Gra}_n$.

\textbf{(i) Bijektivität auf Objekten:} Jede Matrix entspricht genau einem Graphen und umgekehrt (Theorem~\ref{thm:bijektivitaet}).

\textbf{(ii) Funktor-Eigenschaft:} $\Phi_n$ erhält Komposition:
\[
  \Phi_n(\vA \cdot \vB) = \Phi_n(\vA) \circ \Phi_n(\vB)
\]
(Theorem~\ref{thm:erhaltung_operationen}, (iii))

\textbf{(iii) Natürliche Isomorphie:} Alle strukturellen Eigenschaften werden erhalten.

Daher ist $\Phi_n$ ein Äquivalenzfunktor und die beiden Kategorien sind äquivalent.
\end{proof}

\subsection{Warum Induktion in der Topologie fehlschlagen kann}
\label{subsec:anhang_induktion_topologie}

\begin{remark}[Induktion und unendliche Strukturen]
\label{rem:anhang_induktion_unendlich}
Es gibt Szenarien, wo Induktion unvermeidlich ist:

\begin{enumerate}[label=(\alph*)]
  \item \textbf{Unendliche Graphen:} Bei überabzählbaren oder kontinuierlichen Graphen kann man nicht alle Elemente aufzählen. Hier könnte Induktion ein berechtigtes Werkzeug sein.
  
  \item \textbf{Transfinite Induktion:} Für unendliche Strukturen gibt es verallgemeinerte Induktionsprinzipien (über Ordinalzahlen). Diese sind bei unendlichen Objekten legitim.
  
  \item \textbf{Die Position dieser Arbeit:} Wir argumentieren nicht, dass Induktion überall fehlerhaft ist, sondern dass sie für \emph{endliche} Strukturen (Graphen mit endlich vielen Knoten, Matrizen von endlicher Größe) methodisch unangemessen ist.
\end{enumerate}

Für endliche Strukturen ist Deduktion immer möglich und zu bevorzugen.
\end{remark}
...
\url{https://github.com/naphets29/subgraph}.
